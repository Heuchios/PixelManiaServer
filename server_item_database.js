...
if (ITEM_DEFINITIONS["donation_box"])
    ITEM_DEFINITIONS["donation_box"].seed = "donation_box_seed";
...
const SHEET_HARVEST_SECONDS = Object.freeze({
    "vines_2_seed": 100,
    "grass_seed": 100,
    "bush_seed": 100,
    "mushroom_seed": 100,
...
for (const [itemId, tier] of Object.entries(RECIPE_TIERS)) {
    if (!ITEM_DEFINITIONS[itemId]) {
        const atlasDefinition = buildAtlasItemDefinition(itemId, { block, cleanItemId, displayNameForItemId });
        if (atlasDefinition)
            ITEM_DEFINITIONS[itemId] = block({ ...atlasDefinition });
    }
    if (ITEM_DEFINITIONS[itemId])
        ITEM_DEFINITIONS[itemId].recipe_tier = tier;
}
...
const ITEM_DATA_OVERRIDES = require("./item_data_overrides.json");
for (const [itemId, patch] of Object.entries(ITEM_DATA_OVERRIDES)) {
    if (!ITEM_DEFINITIONS[itemId] && patch.category === "seed")
        ITEM_DEFINITIONS[itemId] = seed(String(patch.grows_into), patch);
    if (ITEM_DEFINITIONS[itemId])
        Object.assign(ITEM_DEFINITIONS[itemId], patch);
}
...
function isVendableItem(itemId) {
    ...
    if (clean === "" || clean === "punch" || clean === "small_lock" || clean === "medium_lock" || clean === "big_lock" || clean === "world_lock" || clean === "super_world_lock" || clean === "world_lock_key")
        return false;
    if (clean === "vending_machine" || clean === "vend_empty" || clean === "vend_pending" || clean === "vend_sold")
        return false;
