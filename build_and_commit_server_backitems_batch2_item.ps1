...
Write-Host "== PixelManiaServer: build + commit back-item atlas migration batch 2 (server) ==" -ForegroundColor Cyan
...
$commitMessage = @"
Mirror back-item atlas migration batch 2 (server)

- Mirror the client's item_database.gd changes for carboard_wings, angel_wings,
  void_aura, blue_jetpack, green_jetpack (renamed from purple_jetpack), and
  dragon_fire_wings (renamed from flaming_wings): texture/inventory_icon and
  idle_frames/jump_frames/fall_frames/flap_frames now reference the wearable
  atlas frame keys instead of res://Assets/items/back_items/*.png files.
  sprite_folder/idle_sprite dropped where present.
- Add golden_angel_wings (new item), modeled on angel_wings/devil_wings' 5-frame
  pattern, mirroring the client entry.
- NOTE: green_jetpack and dragon_fire_wings are renamed item ids (previously
  purple_jetpack / flaming_wings). Any live inventories referencing the old ids
