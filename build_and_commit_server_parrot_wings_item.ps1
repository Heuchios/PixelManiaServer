...
Write-Host "== PixelManiaServer: build + commit parrot_wings atlas migration (server) ==" -ForegroundColor Cyan
...
$commitMessage = @"
Make parrot_wings fully atlas-driven (server mirror)

- Mirror the client's item_database.gd change: parrot_wings' texture/inventory_icon and
  idle_frames/jump_frames/fall_frames/flap_frames now reference the wearable atlas frame
  keys (parrot_wings_icon, parrot_wings_1..4) instead of
  res://Assets/items/back_items/parrot_wings_*.png. sprite_folder/idle_sprite dropped,
  matching the client entry. Frame ordering unchanged (only 4 frames, no frame 5, matching
  the original file-based entry).
"@
