...
class PostgresStore {
    ...
    async withTransactionNow(work, label = "transaction") {
        if (!this.isReady())
            return null;
    ...
    async reconcileItemInstancesForInventory(client, playerId, playerState, details = {}) {
        ...
        if (!allowCreateMissing && !allowRetireExtra)
            return;
    ...
    async mirrorWorldDropsState(client, worldId, worldState = {}) {
        ...
        const flush = async () => {
            ...
            const tuples = batch.map((drop) => {
                ...
                values.push(worldId, drop.drop_id, drop.item_type, drop.item_category, drop.amount, drop.x, drop.y, drop.stack_grid_x, drop.stack_grid_y, drop.pickup_delay, JSON.stringify({ ...safeJson(drop.metadata),
      ...
      ON CONFLICT (world_id, drop_id) DO UPDATE SET
        ...
        for (const rawDrop of rawDrops) {
            ...
            if (batchIds.has(drop.drop_id) || batch.length >= 250)
                await flush();
            batch.push(drop);
    ...
    async recordWorldChangesAndTrackedDrops(client, worldId, changes = []) {
        ...
        if (this.questReady) {
            for (const change of pendingChanges) {
                if (change.source_type !== "world_block_update" || change.action !== "break" || change.layer !== "foreground")
                    continue;
                ...
                if (actor.rows[0])
                    await QuestStore.recordGameplay(this, client, actor.rows[0].player_id, "world_block_break", { block_type: change.block_type_before || change.block_type }, String(change.source_id || change.request_id || ""));
            }
        }
