            ...
            try {
                await runTransaction(async (client) => {
                    ...
                    try {
                        ...
                        if (accountId === "") {
                            const playerId = await timeOperation("identity_ensure_ms", () => this.ensurePlayerIdentity(client, username, email, role));
                            if (!playerId)
                                throw new Error("session account identity unavailable");
                            assertPostgresOperationCanContinue(details);
                            const accountResult = await timeOperation("account_lookup_ms", () => client.query(`SELECT account_id FROM ${this.table("accounts")} WHERE lower(username::text) = lower($1) LIMIT 1`, [username]));
                        ...
                        if (rotatedFromTokenHash !== "" && (!rotatedFromSessionId || !tokenFamily)) {
                            const rotatedResult = await timeOperation("rotated_session_lookup_ms", () => client.query(`
                  ...
                  `, [rotatedFromTokenHash]));
                        ...
                        assertPostgresOperationCanContinue(details);
                        await timeOperation("session_upsert_ms", () => client.query(`
                ...
                `, [
                        ...
                        assertPostgresOperationCanContinue(details);
                        if (touchLogin) {
                            await timeOperation("account_touch_ms", () => client.query(`
                  UPDATE ${this.table("accounts")}
                  ...
                  `, [accountId]));
                        ...
                        if (revokeOtherSessions) {
                            await timeOperation("session_revoke_ms", () => client.query(`
                  ...
                  `, [accountId, rotatedFromTokenHash, sessionHash]));
                        ...
                        if (diagnostics)
                            diagnostics.transaction_work_ms = transactionWorkMs;
                    }
                });
            }
            finally {
    ...
    async applyTradeFinalizationTransaction(entry = {}) {
        ...
        try {
          ...
          `, [
            ...
            VALUES ($1, $2, $3, $4, $5, $6)
            `, [tradeId, requesterId, 0, item.item_id, item.item_category || "block", item.amount]).catch(() => { });
                }
