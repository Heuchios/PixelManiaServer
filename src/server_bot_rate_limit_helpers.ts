...
interface RateLimitHelperDeps {
  ...
  // Optional so this module still type-checks and runs for callers that do not
  // wire the ledger (the build-gate harness constructs deps directly); the call
  // site guards on typeof before invoking it.
  recordSecurityViolation?: (subject: string, label: string, context?: PacketRecord) => unknown;
...
function createServerBotRateLimitHelpers(deps: RateLimitHelperDeps) {
  ...
  function getSocketRateLimitSubject(socket: PacketRecord | null | undefined): string {
    return `socket:${socket?.playerId || "unknown"}`;
  }
  ...
  function logRateLimitSecurityEvent(
    ...
    deps.logSecurityEvent(
      ...
      deps.messageRouterHelpers.buildRateLimitSecurityEventDetails(
        ...
        data,
        player,
        subjectOverride || getRateLimitSubject(socket, player)
      ),
  ...
  function recordRateLimitRejection(scope: string, bucketKey: string, subjectKind: string, subject: string = ""): void {
