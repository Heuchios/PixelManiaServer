"use strict";
import Engine = require("./server_quest_engine");
type Row = Record<string, any>;

async function ensureSchema(store:any):Promise<void>{
...
async function apply(store:any, entry:Row):Promise<Row>{
 ...
 if(!username)return {ok:false,message:"Sign in to use the Dispatch."};
 try{
  const result=await store.withTransaction(async(client:any)=>{
   ...
   if(changed.gemDelta){
    ...
    gemsBefore=Number(balance.rows[0].amount);gemsAfter=gemsBefore!+changed.gemDelta;
    if(gemsAfter===null||!Number.isSafeInteger(gemsAfter)||gemsAfter<0||gemsAfter>Number(balance.rows[0].stack_limit))throw new Error("Gem balance cannot accept this reward yet. Your letter is still ready.");
   ...
   if(changed.receipts.length){
    ...
    await store.recordTransactionLedger(client,{transaction_type:changed.gemDelta?"QUEST_REWARD":"QUEST_COSMETIC",player_id:playerId,
     ...
     quantity:changed.gemDelta||changed.receipts.reduce((n:number,r:Row)=>n+r.stamps,0),gems_before:gemsBefore,gems_after:gemsAfter,
  ...
  if(error?.code)store.logger("[quests] operation failed",{action:entry.action,code:error.code,constraint:error.constraint||""});
