...
interface AdminLookupRoutesDeps extends Record<string, any> {}

function toErrorMessage(error: any): string {
  return error && error.message ? String(error.message) : String(error || "unknown");
}

function createServerAdminLookupRoutes(deps: AdminLookupRoutesDeps) {
  const {
  ...
  function buildAdminInventoryLookupPlayerData(username: unknown, state: unknown): PacketRecord {
    const clean = cleanAccountName(username);
    const source = state && typeof state === "object" && !Array.isArray(state)
      ? state as PacketRecord
  ...
  function sendAdminInventoryLookupFailure(socket: unknown, requestId: unknown, targetUsername: unknown, message: unknown, extra: PacketRecord = {}): void {
    sendJson(socket, {
      type: "player_state",
      ok: false,
      found: false,
  ...
  function sendAdminItemInstanceLookupFailure(socket: unknown, requestId: unknown, targetUsername: unknown, message: unknown, extra: PacketRecord = {}): void {
    sendJson(socket, {
      type: "player_state",
      ok: false,
      found: false,
  ...
  function sendAdminItemInstanceHistoryLookupFailure(
    socket: unknown,
    requestId: unknown,
  ...
  function sendAdminTransactionLedgerLookupFailure(
    socket: unknown,
    requestId: unknown,
    targetUsername: unknown,
    itemInstanceId: unknown,
  ...
  function sendAdminMonitoringDashboardFailure(socket: unknown, requestId: unknown, targetUsername: unknown, message: unknown, extra: PacketRecord = {}): void {
    sendJson(socket, {
      type: "player_state",
      ok: false,
      found: false,
  ...
  function buildAdminMonitoringOnlinePlayers(limit = 100): PacketRecord[] {
  ...
  function buildAdminMonitoringWorldRows(limit = 100): PacketRecord[] {
    const worldRows: PacketRecord[] = [];
    const playerCounts = new Map<string, number>();
    for (const worldName of Array.from(worldPlayers.keys())) {
      const count = getWorldPopulationCount(worldName);
      if (count > 0) playerCounts.set(cleanWorld(worldName), count);
    }
  ...
  function buildAdminMonitoringRuntimeSnapshot(limit = 100): PacketRecord {
    ...
    return {
      ...
      authenticated_player_count: Array.from(players.values()).filter((player) => Boolean((player as PacketRecord)?.authenticated)).length,
  ...
  function buildAdminItemInstanceLookupRows(itemInstances: unknown): PacketRecord[] {
    const rows = Array.isArray(itemInstances) ? itemInstances : [];
    return rows.map((entry: PacketRecord) => ({
  ...
  function buildAdminTransactionLedgerLookupRows(entries: unknown): PacketRecord[] {
  ...
  function handleAdminInventoryLookupRequest(socket: unknown, player: PacketRecord, data: PacketRecord, username: unknown, requestId: unknown, purpose: unknown): void {
    ...
    const deny = (message: string, details: PacketRecord = {}, extra: PacketRecord = {}) => {
  ...
  async function handleAdminItemInstanceLookupRequest(socket: unknown, player: PacketRecord, data: PacketRecord, username: unknown, requestId: unknown, purpose: unknown): Promise<void> {
    const targetUsername = cleanAccountName(data.target_username || data.requested_username || username);
    const logBase = {
      request_id: requestId,
      purpose,
  ...
  async function handleAdminItemInstanceHistoryLookupRequest(socket: unknown, player: PacketRecord, data: PacketRecord, username: unknown, requestId: unknown, purpose: unknown): Promise<void> {
    const targetUsername = cleanAccountName(data.target_username || data.requested_username || username);
  ...
  async function handleAdminTransactionLedgerLookupRequest(socket: unknown, player: PacketRecord, data: PacketRecord, username: unknown, requestId: unknown, purpose: unknown): Promise<void> {
  ...
  async function handleAdminMonitoringDashboardRequest(socket: unknown, player: PacketRecord, data: PacketRecord, username: unknown, requestId: unknown, purpose: unknown): Promise<void> {
