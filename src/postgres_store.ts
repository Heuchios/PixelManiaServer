...
type DetachedWork = () => Promise<unknown>;
type WorldPersistenceMetadata = {
...
type ItemInstanceMovement = RuntimeRecord;
...
let PoolClass: PostgresPoolConstructor | null = null;
try {
  PoolClass = (require("pg") as { Pool: PostgresPoolConstructor }).Pool;
} catch {
  PoolClass = null;
}
...
const INVENTORY_FIELD_BY_CATEGORY = new Map(
  INVENTORY_FIELD_CATEGORY.map(([field, category]) => [category, field])
);
...
function postgresError(error: unknown): PostgresError {
  ...
  const normalized = new Error(String(error || "Unknown PostgreSQL error")) as PostgresError;
  if (error && typeof error === "object") {
    const details = error as RuntimeRecord;
    for (const field of ["code", "column", "constraint", "detail", "schema", "table"] as const) {
...
function resultForTrackedItemMovementError(
  error: unknown,
  fallbackReason = "tracked_item_instance_movement_failed"
): RuntimeRecord | null {
  const trackedError = error as RuntimeRecord | null;
...
function toObject(value: unknown): RuntimeRecord {
...
function normalizeWorldPersistenceMetadata(value: unknown, worldState: RuntimeRecord = {}): WorldPersistenceMetadata {
...
function worldStateForPersistenceChecksum(value: unknown): RuntimeRecord {
...
function worldPersistenceChecksum(value: unknown): string {
  return jsonChecksum(worldStateForPersistenceChecksum(value));
}

function normalizeDbRole(value: unknown): string {
  const role = cleanName(value).toLowerCase();
  if (role === "developer") return "admin";
  if (role === "mod") return "moderator";
  if (role === "player" || role === "moderator" || role === "designer" || role === "admin" || role === "owner") return role;
  return "player";
}
...
function buildTransactionLedgerHashPayload(entry: any = {}) {
  return PostgresContracts.buildTransactionLedgerHashPayload(entry);
}

function buildTransactionLedgerHash(entry: any = {}) {
...
function applyCanonicalInventoryRowsToPlayerState(
  rawState: unknown,
  inventoryRows: RuntimeRecord[] = []
): RuntimeRecord {
  ...
  for (const rawRow of rows) {
    ...
    if (itemType === "" || !inventoryField || amount <= 0) continue;
 ...
 */
function normalizeWorldDropPayload(drop: any = {}, fallback: any = {}) {
 ...
 */
function worldDropRowToPayload(row: any = {}) {
  return PostgresContracts.worldDropRowToPayload(row);
}

function worldLockRowToPayload(row: any = {}) {
 ...
 * @returns {boolean}
 */
function isServerAuthoredWorldDropOrigin(source: unknown, action: unknown): boolean {
  const cleanSource = cleanName(source).toLowerCase();
  const cleanAction = cleanName(action).toLowerCase();
  if (cleanSource === "" && cleanAction === "") return false;
  if (cleanSource.includes("drop_inventory") || cleanSource.includes("world_item_drop_create")) return false;
  if (cleanAction.includes("drop_inventory") || cleanAction.includes("world_item_drop_create")) return false;
  if (
    cleanSource.includes("world_block_break") ||
...
function normalizeTransactionLedgerType(entry: any = {}) {
...
function summarizeItemInstanceEventMetadata(value: unknown): RuntimeRecord {
...
function normalizeProgressionState(state: unknown): RuntimeRecord {
  return PostgresContracts.normalizeProgressionState(state);
}

function safeJson(value: unknown): RuntimeRecord {
  return PostgresContracts.safeJson(value);
}

function clonePlainJson<T>(value: T): T {
  return PostgresContracts.clonePlainJson(value) as T;
}
...
function normalizeWorldObjectType(entry: any = {}) {
  return PostgresContracts.normalizeWorldObjectType(entry);
}

function normalizeWorldObjectId(entry: any = {}, worldName = "", objectType = "") {
  return PostgresContracts.normalizeWorldObjectId(entry, worldName, objectType);
}

function shouldTreatAsWorldObjectChange(entry: any = {}) {
  return PostgresContracts.shouldTreatAsWorldObjectChange(entry);
}

function extractWorldObjectJournalMap(worldState: any = {}, fallbackWorldName = "") {
...
class PostgresStore {
  ...
  async queryReadWithRetry(label: unknown, query: string, values: any[] = []): Promise<QueryResult<any>> {
    ...
    for (let attempt = 1; attempt <= POSTGRES_READ_MAX_ATTEMPTS; attempt += 1) {
      try {
        ...
        this.logger(
          `[postgres] ${cleanLabel} attempt ${attempt} hit a transient database conflict; retrying in ${retryDelay}ms.`,
  ...
  async applyBootstrapSql(sqlPath: string) {
  ...
  // server_landfill_event.ts and project memory landfill_seasonal_event_design.md). Keyed by
  // player_id (uuid), matching the convention used by every other per-player table in this file
  // (gem_ledger, world_honor_visits, transaction_ledger, player_progression_events, ...) rather
  // than by username text -- callers resolve username -> player_id via
  // ensurePlayerIdentityForExistingAccount inside the same transaction as the write, and reads
  // join players/accounts back to a display username.
  //
  //   landfill_season_scores -- one row per (player_id, season_key): the cumulative lifetime-
  //                             per-season Kilogram total the leaderboard ranks on.
  //   landfill_prize_claims  -- one row per (player_id, season_key): claim receipt so a top-10
  //                             finisher can only claim their season's prize once.
  //   landfill_race_results  -- one row per (session_id, player_id): exactly-once idempotency
  //                             guard for recordLandfillRaceResult (a duplicate call must insert
   ...
   * INSERT simply returns no row, and the increment is skipped with it. persistSessionResults
   * already wraps its call to this in a try/catch, but this also catches internally so a database
   * hiccup here degrades to a logged warning rather than relying solely on the caller.
   */
  async recordLandfillRaceResult(
    entry: unknown
  ): Promise<{ ok: boolean; reason?: string; recorded?: boolean; duplicate?: boolean; awarded_kilograms?: number }> {
    if (!this.isReady() || !this.landfillReady) return { ok: false, reason: "postgres_unavailable" };
    const record = (entry || {}) as Record<string, unknown>;
    const sessionId = cleanName(record.sessionId ?? record.session_id ?? "");
    const cleanUsername = cleanName(record.username ?? "");
    const cleanSeasonKey = cleanName(record.seasonKey ?? record.season_key ?? "");
    const worldName = cleanName(record.worldName ?? record.world_name ?? "");
    const kilograms = Math.max(0, toInt(record.kilograms, 0));
    const placement = Math.max(1, toInt(record.placement, 1));
    const awardedKilograms = Math.max(0, toInt(record.awardedKilograms ?? record.awarded_kilograms, 0));
  ...
  // Standalone KG credit not tied to a race-result row. Kept for parity with the original
  // implementation this restores -- recordLandfillRaceResult (via the session's PRIMARY KEY) is
  // the idempotency-safe path server_landfill_event.ts actually uses for race rewards; a caller
  // of this method is responsible for its own duplicate-call protection.
  async incrementLandfillKilograms(
    username: unknown,
    seasonKey: unknown,
    amount: unknown
  ): Promise<{ ok: boolean; reason?: string; kilograms?: number }> {
    if (!this.isReady() || !this.landfillReady) return { ok: false, reason: "postgres_unavailable" };
  ...
  // Top-N for the season, ranked by cumulative kilograms. Joins players/accounts back to a
  // display username since landfill_season_scores itself only stores player_id. Ties break on
  // updated_at ASC (whoever reached that total first), same tie-break discipline
  // server_landfill_event.ts uses for in-race ranking.
  async getLandfillLeaderboard(seasonKey: unknown, limit: unknown = 10): Promise<{ ok: boolean; entries: any[]; reason?: string }> {
    if (!this.isReady() || !this.landfillReady) return { ok: false, entries: [], reason: "postgres_unavailable" };
    const cleanSeasonKey = cleanName(seasonKey);
    const cleanLimit = Math.max(1, Math.min(50, toInt(limit, 10)));
    if (cleanSeasonKey === "") return { ok: false, entries: [], reason: "invalid_input" };
  ...
  // who simply hasn't scored yet gets { kilograms: 0, rank: 0 }, matching
  // handleLandfillLeaderboardRequest's `standing?.kilograms || 0` / `standing?.rank || 0` fallback.
  async getLandfillPlayerScore(username: unknown, seasonKey: unknown): Promise<{ kilograms: number; rank: number } | null> {
    if (!this.isReady() || !this.landfillReady) return null;
    const cleanUsername = cleanName(username);
    const cleanSeasonKey = cleanName(seasonKey);
    if (cleanUsername === "" || cleanSeasonKey === "") return null;
    ...
    try {
      ...
      if (!result.rows[0]) return { kilograms: 0, rank: 0 };
  ...
  async insertLandfillPrizeClaim(username: unknown, seasonKey: unknown, rank: unknown): Promise<{ ok: boolean; inserted: boolean; reason?: string }> {
    if (!this.isReady() || !this.landfillReady) return { ok: false, inserted: false, reason: "postgres_unavailable" };
    const cleanUsername = cleanName(username);
    const cleanSeasonKey = cleanName(seasonKey);
    ...
    try {
      const result = await this.withTransaction(async (client: any) => {
        const playerId = await this.ensurePlayerIdentityForExistingAccount(client, cleanUsername);
        if (!playerId) return { ok: false, inserted: false, reason: "identity_missing" };
  ...
  // Never throws -- claimLandfillPrize calls this from failure branches with no return-value
  // check, same contract as deleteWorldState.
  async deleteLandfillPrizeClaim(username: unknown, seasonKey: unknown): Promise<{ ok: boolean }> {
    if (!this.isReady() || !this.landfillReady) return { ok: false };
    const cleanUsername = cleanName(username);
    const cleanSeasonKey = cleanName(seasonKey);
    if (cleanUsername === "" || cleanSeasonKey === "") return { ok: false };
  ...
  enqueueWrite<T>(
    ...
    if (this.writeQueueDepth >= this.maxWriteQueueDepth) {
      const error: any = new Error(`write queue is full while scheduling ${cleanLabel}`);
  ...
  async lookupPlayerIdByUsername(client: PoolClient, username: unknown) {
    const cleanUsername = cleanName(username);
    if (cleanUsername === "") return null;
  ...
  async lookupPlayerIdentityByUsername(username: unknown) {
    if (!this.isReady()) return null;
    const cleanUsername = cleanName(username);
    if (cleanUsername === "") return null;
    try {
  ...
  async lookupPlayerIdByAccountId(client: PoolClient, accountId: string) {
    const cleanAccountId = cleanName(accountId);
    if (!isUuid(cleanAccountId)) return null;
    const result = await client.query(
      `
      SELECT player_id
  ...
  async lookupAccountIdByPlayerId(client: PoolClient, playerId: string) {
    const cleanPlayerId = cleanName(playerId);
    if (!isUuid(cleanPlayerId)) return null;
    const result = await client.query(
      `
      SELECT account_id
        FROM ${this.table("players")}
  ...
  async lookupUsernameByPlayerId(client: PoolClient, playerId: string) {
    const cleanPlayerId = cleanName(playerId);
  ...
  async claimIdempotency(scope: unknown, key: unknown, username = "", metadata: any = {}, options: any = {}) {
  ...
  async upsertAccountState(client: PoolClient, account: RuntimeRecord, options: any = {}) {
  ...
  async saveAccountState(account: RuntimeRecord, options: any = {}) {
  ...
  async loadAccountState(username: unknown) {
    if (!this.isReady()) return null;
    const cleanUsername = cleanName(username);
    if (cleanUsername === "") return null;
  ...
  async replaceInventorySnapshot(client: PoolClient, playerId: string, playerState: RuntimeRecord) {
    if (!playerId) return;
  ...
  async reconcileItemInstancesForInventory(
    client: PoolClient,
    playerId: string,
    playerState: RuntimeRecord,
    details: any = {}
  ) {
    if (!playerId) return;
    ...
    const activeByItem = new Map<string, RuntimeRecord[]>();
  ...
  async recordItemInstanceEvent(client: PoolClient, entry: any = {}) {
  ...
  async getInventorySnapshotHash(client: PoolClient | Pool, playerId: string) {
  ...
  async updatePlayerInventoryHash(
    client: PoolClient | Pool,
    playerId: string,
    inventoryHash: string | null = null
  ) {
    if (!isUuid(playerId)) return null;
  ...
  async recordTransactionLedger(client: PoolClient, entry: any = {}) {
    const e = toObject(entry);
    ...
    const insertedIds: Array<string | number | null> = [];
  ...
  async recordTransactionLedgerEvent(entry: any = {}) {
  ...
  async listTransactionLedger(entry: any = {}) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    const e = toObject(entry);
  ...
  async createItemInstance(entry: any = {}) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    const e = toObject(entry);
    const itemType = cleanName(e.item_type || e.item_id || "");
  ...
  async updateItemInstance(entry: any = {}) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    const e = toObject(entry);
    const itemInstanceId = cleanName(e.item_instance_id || e.instance_id || "");
  ...
  async listActiveItemInstances(username: unknown, options: any = {}) {
    if (!this.isReady()) return [];
    const cleanUsername = cleanName(username);
  ...
  async listItemInstanceCopies(identifier: unknown, options: any = {}) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    ...
    let anchor: RuntimeRecord | null = null;
  ...
  async getItemInstanceHistory(identifier: unknown, options: any = {}) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    const cleanIdentifier = cleanName(identifier);
    if (cleanIdentifier === "") return { ok: false, reason: "invalid_item_instance" };
  ...
  async adjustInventoryForItemInstanceModeration(client: PoolClient, entry: any = {}) {
    const e = toObject(entry);
  ...
  async moderateItemInstance(identifier: unknown, action: unknown, options: any = {}) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    ...
    try {
      return await this.withTransaction(async (client) => {
        ...
        const inventoryEffects: RuntimeRecord[] = [];
        ...
        let eventType = "updated";
        let eventTransactionId: number | null = null;

        const moderationMetadata: any = {
        ...
        const applyInventoryEffect = async (playerId: string, delta: number) => {
  ...
  async auditItemInstances(options: any = {}) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    const limit = Math.min(200, Math.max(1, toInt(options.limit, 50)));
    const issues: RuntimeRecord[] = [];
    const scannedAt = new Date().toISOString();

    const pushIssue = (issue: RuntimeRecord): void => {
  ...
  async auditIntegrityHashes(options: any = {}) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    const limit = Math.min(500, Math.max(1, toInt(options.limit, 100)));
  ...
  async getAdminMonitoringDashboard(options: any = {}) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    ...
    try {
      ...
      const latestAuditIssues: RuntimeRecord[] = latestAuditRow && Array.isArray(latestAuditRow.issues)
      ...
      const itemAuditIssues: RuntimeRecord[] = itemAudit?.ok && Array.isArray(itemAudit.issues) ? itemAudit.issues : [];
  ...
  async reconcileItemInstancesForUsername(
    username: unknown,
    playerState: RuntimeRecord | null = null,
    details: any = {}
  ) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    const cleanUsername = cleanName(username);
  ...
  async reconcileStoredItemInstancesFromPlayerStates() {
    ...
    try {
      ...
      if ((rows.rowCount ?? 0) <= 0) return { ok: true, player_count: 0 };
  ...
  async savePlayerState(username: unknown, state: RuntimeRecord) {
    if (!this.isReady()) return false;
  ...
  async loadPlayerState(username: unknown) {
    if (!this.isReady()) return { ok: false, found: false, reason: "postgres_unavailable" };
    const cleanUsername = cleanName(username);
    if (cleanUsername === "") return { ok: false, found: false, reason: "invalid_username" };
    ...
    try {
      ...
      const row = result.rows[0];
      if (!row) return { ok: true, found: false, username: cleanUsername };
  ...
  async loadPlayerStates() {
    ...
    try {
      ...
      const inventoryResult = playerIds.length > 0
        ? await this.queryReadWithRetry(
          "player inventories load",
          `
      ...
      return result.rows.map((row) => {
        const state = applyCanonicalInventoryRowsToPlayerState(
          row.player_state,
          inventoryRowsByPlayerId.get(cleanName(row.player_id || "")) || []
        );
        return {
          username: cleanName(row.username),
          state: {
  ...
  async ensureWorldIdentity(client: PoolClient, worldName: unknown) {
  ...
  logWorldPersistence(event: unknown, details: RuntimeRecord = {}): void {
   ...
   */
  async getWorldOwnerEpoch(worldName: unknown): Promise<number> {
    const cleanWorldName = cleanName(worldName || "");
    if (cleanWorldName === "" || !this.isReady()) return 0;
    try {
      const result = await this.queryReadWithRetry(
        "world owner epoch load",
        `SELECT world_owner_epoch FROM ${this.table("worlds")} WHERE world_name = $1`,
        [cleanWorldName]
      );
  ...
  async claimWorldPersistenceOwnership(worldName: unknown, ownership: unknown = {}): Promise<WorldPersistenceResult> {
    if (!this.isReady()) {
      return {
        ok: false,
    ...
    try {
      const result = await this.withTransaction(async (client): Promise<WorldPersistenceResult> => {
        await client.query(
          `
  ...
  async upsertWorldState(
    client: PoolClient,
    worldName: unknown,
    worldState: RuntimeRecord,
    ownership: unknown = {}
    ...
    const rowHasOwnershipFence = Boolean(
      normalizeWorldRevision(row?.world_owner_epoch) > 0
      || cleanName(row?.world_owner_token || "") !== ""
      || cleanName(row?.world_owner_instance || "") !== ""
    );
    if (rowHasOwnershipFence && !metadata.require_owner) {
      const rejected: WorldPersistenceResult = {
        ok: false,
    ...
    if (row && requestedRevision < persistedRevision) {
      const stale: WorldPersistenceResult = {
        ok: false,
    ...
    const saveResult: WorldPersistenceResult = {
      ok: saved,
   ...
   */
  async upsertWorldDropRow(client: PoolClient, worldId: string, drop: any = {}, options: any = {}) {
    if (!worldId) return { ok: false, reason: "missing_world" };
    const normalized = normalizeWorldDropPayload(drop, options);
    if (!normalized) return { ok: false, reason: "invalid_drop" };
   ...
   */
  async mirrorWorldDropsState(client: PoolClient, worldId: string, worldState: any = {}) {
    if (!worldId) return { ok: false, reason: "missing_world" };
  ...
  async mirrorWorldLockState(client: PoolClient, worldId: string, worldState: RuntimeRecord) {
    if (!worldId) return;
    const state = toObject(worldState);
    ...
    // Rebuild identity access from the current member list so removed members
    // cannot retain grants through a stale account/player-id map.
    const allowedAccountIdSet = new Set<string>();
    const allowedPlayerIdSet = new Set<string>();
    const rolesByAccountId: RuntimeRecord = {};
    const rolesByPlayerId: RuntimeRecord = {};
    ...
    if (ownerPlayerId) {
      await client.query(
        `DELETE FROM ${this.table("world_members")} WHERE world_id = $1 AND role = 'owner' AND player_id <> $2`,
  ...
  async mirrorWorldAreaLocksState(client: PoolClient, worldId: string, worldState: RuntimeRecord) {
    if (!worldId) return;
    const state = toObject(worldState);
    ...
    for (const rawLock of rawLocks) {
      ...
      if (lockType !== "small_lock" && lockType !== "medium_lock" && lockType !== "big_lock") continue;
  ...
  async saveWorldState(worldName: unknown, state: RuntimeRecord, ownership: unknown = {}) {
   ...
   */
  async insertWorldBlockChange(client: PoolClient, worldId: string, entry: any = {}) {
   ...
   */
  async buildWorldBlockChangeRowValues(
    client: PoolClient,
    worldId: string,
    entry: any = {}
  ): Promise<any[] | null> {
   ...
   * @returns {string}
   */
  buildWorldBlockChangeInsertStatement(rowCount: number): string {
    const tuples: string[] = [];
    for (let row = 0; row < rowCount; row += 1) {
      const base = row * WORLD_BLOCK_CHANGE_COLUMN_COUNT;
      tuples.push(
        `($${base + 1}, $${base + 2}, $${base + 3}, NULLIF($${base + 4}, ''), $${base + 5}, `
  ...
  isWorldObjectChangeEntry(entry: any = {}) {
   ...
   * @returns {Promise<PixelMania.PostgresWorldChangeInsertResult>}
   */
  async insertWorldObjectChange(client: PoolClient, worldId: string, entry: any = {}) {
   ...
   * @returns {Promise<any[] | null>}
   */
  async buildWorldObjectChangeRowValues(
    client: PoolClient,
    worldId: string,
    entry: any = {}
  ): Promise<any[] | null> {
   ...
   * @returns {string}
   */
  buildWorldObjectChangeInsertStatement(rowCount: number): string {
    const tuples: string[] = [];
    for (let row = 0; row < rowCount; row += 1) {
   ...
   * @returns {Promise<PixelMania.PostgresWorldChangeInsertResult>}
   */
  async recordWorldChangeEntry(client: PoolClient, worldId: string, entry: any = {}) {
   ...
   */
  async recordWorldChangeAndTrackedDrops(client: PoolClient, worldId: string, change: any = {}) {
   ...
   * @returns {Promise<void>}
   */
  async recordTrackedDropsForWorldChange(client: PoolClient, worldId: string, change: any = {}) {
   ...
   * @returns {Promise<void>}
   */
  async recordWorldChangesAndTrackedDrops(client: PoolClient, worldId: string, changes: any[] = []) {
  ...
  async loadWorldStateForUpdate(client: PoolClient, worldName: unknown) {
    const cleanWorldName = cleanName(worldName || "");
    ...
    if (row) state.world_revision = Math.max(
      normalizeWorldRevision(state.world_revision),
      normalizeWorldRevision(row.world_revision)
    );
    return state;
  }
  ...
  // rejection.
  async deleteWorldState(worldName: unknown): Promise<{ ok: boolean; reason?: string; deleted?: boolean }> {
    const cleanWorldName = cleanName(worldName || "");
    if (cleanWorldName === "") return { ok: false, reason: "invalid_world" };
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
  ...
  async loadWorldState(worldName: unknown) {
    const cleanWorldName = cleanName(worldName || "");
    if (cleanWorldName === "") return { ok: false, found: false, reason: "invalid_world" };
    if (!this.isReady()) {
    ...
    try {
      ...
      const state: RuntimeRecord = {
        ...
        world_revision: Math.max(
          normalizeWorldRevision(rawWorldState.world_revision),
          normalizeWorldRevision(row.world_revision)
        ),
      };
  ...
  buildWorldObjectChangesFromStateDiff(beforeState: any = {}, afterState: any = {}, context: any = {}) {
   ...
   */
  async saveWorldStateWithWorldChanges(
    worldName: unknown,
    state: RuntimeRecord,
    changes: RuntimeRecord[] = [],
    ownership: unknown = {}
  ) {
    const cleanWorldName = cleanName(worldName || state?.world_name || "START") || "START";
    ...
    try {
      ...
      if (worldNames.length > 0) {
        ...
        for (const row of activeDropRows.rows) {
          const worldName = cleanName(row.world_name || "START") || "START";
          if (!activeDropsByWorld.has(worldName)) activeDropsByWorld.set(worldName, []);
      ...
      return result.rows.map((row) => {
        ...
        const state: RuntimeRecord = {
          ...
          world_revision: Math.max(
            normalizeWorldRevision(toObject(row.world_state).world_revision),
            normalizeWorldRevision(row.world_revision)
          ),
        };
  ...
  async listOwnedWorldLocks(username: unknown, limit = 100, identity: any = {}) {
    if (!this.isReady()) return [];
    ...
    if (ownerName === "" && ownerAccountId === "" && ownerPlayerId === "") return [];
   ...
   */
  async loadActiveWorldDrops(worldName: unknown) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable", drops: [] };
    const cleanWorldName = cleanName(worldName || "");
    if (cleanWorldName === "") return { ok: false, reason: "invalid_world", drops: [] };
  ...
  async saveWorldSnapshot(worldName: unknown, snapshot: RuntimeRecord, options: any = {}) {
    if (!this.isReady()) return false;
    ...
    try {
      const result = await this.withTransaction(async (client) => {
        ...
        if (!row) return { ok: false, reason: "world_not_found", affected_rows: 0, persisted_revision: 0 };
        if (
        ...
        snapshotData.world_revision = Math.max(
          normalizeWorldRevision(snapshotData.world_revision),
          normalizeWorldRevision(row.world_revision)
        );
  ...
  mirrorAdminAction(entry: RuntimeRecord) {
    if (!this.isReady()) return;
    const e = toObject(entry);
  ...
  mirrorAccount(account: RuntimeRecord, options: any = {}) {
    if (!this.isReady()) return;
    const accountData = toObject(account);
  ...
  async saveSession(account: RuntimeRecord, details: any = {}) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    const accountData = toObject(account);
    const username = cleanName(accountData.username);
  ...
  mirrorSession(account: RuntimeRecord, details: any = {}) {
    return this.saveSession(account, details);
  }

  async validateSessionToken(username: unknown, sessionTokenHash: unknown, details: any = {}) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    const cleanUsername = cleanName(username);
  ...
  async revokeSessionsForUsername(username: unknown, reason = "revoked") {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    const cleanUsername = cleanName(username);
    if (cleanUsername === "") return { ok: false, reason: "invalid_username" };
  ...
  revokeSessionsByUsername(username: unknown) {
  ...
  async revokeOtherSessionsForUsername(
    username: unknown,
    keepSessionTokenHash: unknown,
    reason = "one_active_session"
  ) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    const cleanUsername = cleanName(username);
  ...
  async revokeSessionByTokenHash(sessionTokenHash: unknown, reason = "revoked") {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    const cleanSessionHash = cleanName(sessionTokenHash);
  ...
  async createAccountPasswordResetRequest(entry: any = {}) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    const e = toObject(entry);
    ...
    try {
      return await this.withTransaction(async (client) => {
        ...
        const accountId = accountResult.rows[0]?.account_id;
        if (!accountId) return { ok: false, reason: "account_not_found" };
  ...
  async consumeAccountPasswordResetRequest(tokenHash: unknown) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    const cleanTokenHash = cleanName(tokenHash);
    if (cleanTokenHash === "") return { ok: false, reason: "missing_token" };
    ...
    try {
      return await this.withTransaction(async (client) => {
        ...
        await client.query(
          `UPDATE ${this.table("account_password_reset_requests")} SET used_at = now() WHERE reset_request_id = $1`,
  ...
  async consumeAccountEmailVerificationToken(tokenHash: unknown) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    const cleanTokenHash = cleanName(tokenHash);
    if (cleanTokenHash === "") return { ok: false, reason: "missing_token" };
  ...
  async resetAccountPasswordWithToken(tokenHash: unknown, passwordData: any = {}) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    const cleanTokenHash = cleanName(tokenHash);
  ...
  async createAccountEmailChangeRequest(entry: any = {}) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    const e = toObject(entry);
    const username = cleanName(e.username || "");
  ...
  async consumeAccountEmailChangeRequest(tokenHash: unknown) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    const cleanTokenHash = cleanName(tokenHash);
    if (cleanTokenHash === "") return { ok: false, reason: "missing_token" };
    ...
    try {
      return await this.withTransaction(async (client) => {
        ...
        await client.query(
          `UPDATE ${this.table("account_email_change_requests")} SET used_at = now() WHERE email_change_request_id = $1`,
          [row.email_change_request_id]
        );
  ...
  async updateAccountPassword(username: unknown, passwordData: any = {}) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    const cleanUsername = cleanName(username);
    const salt = cleanName(passwordData.salt || "");
    const hash = String(passwordData.hash || "");
    ...
    try {
      ...
      if ((result.rowCount ?? 0) < 1) return { ok: false, reason: "account_not_found" };
      return { ok: true };
    } catch (error) {
  ...
  async updateAccountEmail(username: unknown, newEmail: unknown) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    const cleanUsername = cleanName(username);
  ...
  mirrorPlayerWorld(username: unknown, worldName: unknown) {
    if (!this.isReady()) return;
    const cleanUsername = cleanName(username);
  ...
  async updatePlayerProgression(client: PoolClient, playerId: string, state: RuntimeRecord) {
  ...
  mirrorPlayerProgression(username: unknown, state: RuntimeRecord, event: RuntimeRecord = {}) {
  ...
  mirrorInventorySnapshot(username: unknown, state: RuntimeRecord) {
    if (!this.isReady()) return;
    const cleanUsername = cleanName(username);
    const playerState = toObject(state);
    if (cleanUsername === "") return;
  ...
  getItemInstanceLedgerDestination(source: unknown, action: unknown, delta: number) {
   ...
   * @returns {boolean}
   */
  shouldCreateTrackedWorldDropItemInstancesForChange(change: RuntimeRecord = {}) {
  ...
  async syncItemInstancesForLedger(client: PoolClient, entry: any = {}) {
    const e = toObject(entry);
    const playerId = cleanName(e.player_id || "");
    const itemType = cleanName(e.item_type || e.item_id || "");
    const itemCategory = resolveItemCategory(itemType, e.item_category || e.category || "");
    ...
    if (delta > 0) {
      ...
      const createdInstances: ItemInstanceMovement[] = [];
    ...
    const movedInstances: ItemInstanceMovement[] = [];
  ...
  async transferTrackedItemInstances(client: PoolClient, entry: any = {}) {
    const e = toObject(entry);
    ...
    const transferredInstances: ItemInstanceMovement[] = [];
    for (const row of rows.rows) {
      await client.query(
        `
        UPDATE ${this.table("item_instances")}
  ...
  async claimTrackedWorldDropItemInstances(client: PoolClient, entry: any = {}) {
    const e = toObject(entry);
    ...
    const claimedInstances: ItemInstanceMovement[] = [];
    for (const row of rows.rows) {
      await client.query(
        `
        UPDATE ${this.table("item_instances")}
  ...
  async createTrackedWorldDropItemInstances(client: PoolClient, entry: any = {}) {
    const e = toObject(entry);
    const itemType = cleanName(e.item_type || e.item_id || "");
    const itemCategory = resolveItemCategory(itemType, e.item_category || e.category || "");
   ...
   */
  async applyInventoryDeltaTransaction(entry: any = {}) {
    if (!this.isReady()) {
    ...
    try {
      return await this.withTransaction(async (client) => {
        ...
        for (const deltaEntry of deltas) {
          ...
          let repairedFromAmount: number | null = null;
          ...
          let gemLedgerId: number | null = null;
  ...
  mirrorItemLedger(entry: RuntimeRecord) {
    if (!this.isReady()) return;
    const e = toObject(entry);
  ...
  mirrorGemLedger(entry: RuntimeRecord) {
    if (!this.isReady()) return;
    const e = toObject(entry);
    const username = cleanName(e.account_username);
    if (username === "") return;
  ...
  mirrorShopPurchase(entry: RuntimeRecord) {
    if (!this.isReady()) return;
    const e = toObject(entry);
    const username = cleanName(e.account_username);
    if (username === "") return;
  ...
  mirrorTradeTransaction(entry: RuntimeRecord) {
    if (!this.isReady()) return;
    const e = toObject(entry);
  ...
  mirrorVendingTransaction(entry: RuntimeRecord) {
    if (!this.isReady()) return;
    const e = toObject(entry);
   ...
   */
  async applyDropPickupTransaction(entry: any = {}) {
    if (!this.isReady()) {
    ...
    try {
      return await this.withTransaction(async (client) => {
        const playerId = await this.ensurePlayerIdentity(client, username);
        if (!playerId) return DropContracts.buildPostgresDropPickupFailure({ reason: "player_not_found" });
        ...
        if (Object.keys(worldState).length > 0) {
          ...
          if (!persistedWorld.ok || !persistedWorld.world_id) {
            const persistenceError = new Error(
              persistedWorld.reason || "world_state_save_failed"
            ) as Error & {
              code?: string;
              world_persistence_result?: WorldPersistenceResult;
            };
  ...
  async applyTradeFinalizationTransaction(entry: any = {}) {
    if (!this.isReady()) {
      return { ok: false, reason: "postgres_unavailable" };
    }
    ...
    const sanitizeOfferEntries = (offerItems: unknown): TradeOfferEntry[] => {
      const result: TradeOfferEntry[] = [];
    ...
    const buildBaselineMap = (baselineItems: unknown): Map<string, RuntimeRecord> => {
      const baseline = new Map<string, RuntimeRecord>();
    ...
    const isWorldLockKeyTradeItem = (item: unknown): boolean => {
      const parsed = toObject(item);
      return cleanName(parsed.item_id || parsed.item_type || "") === "world_lock_key";
    };

    const buildDeltaMap = (offerItems: TradeOfferEntry[], options: any = {}): Map<string, number> => {
      const deltas = new Map<string, number>();
      for (const item of offerItems) {
        if (options.skipWorldLockKeys === true && isWorldLockKeyTradeItem(item)) continue;
    ...
    const buildNetMap = (
      negativeDeltas: Map<string, number>,
      positiveDeltas: Map<string, number>
    ): Map<string, number> => {
    ...
    const applyInventoryDeltas = async (
      ...
      baselineMap: Map<string, RuntimeRecord>
    ...
    try {
      return await this.withTransaction(async (client) => {
        ...
        if (!requesterInventory.ok) return requesterInventory;
        const targetInventory = await applyInventoryDeltas(client, targetId, netTarget, targetBaseline);
        if (!targetInventory.ok) return targetInventory;
        ...
        const trackedInstanceMovements: ItemInstanceMovement[] = [];
        ...
        const tradeLedgerContextByEntry = new Map<string, RuntimeRecord>();
        const tradeLedgerEntryKey = (playerId: string, entry: InventoryLedgerEntry): string =>
          `${playerId}\u0000${entry.item_type}\u0000${entry.item_category}\u0000${entry.delta}`;
        const rememberTradeLedgerContext = (
          playerId: string,
        ...
        const getTradeLedgerContext = (
          playerId: string,
          entry: InventoryLedgerEntry
        ): RuntimeRecord => tradeLedgerContextByEntry.get(tradeLedgerEntryKey(playerId, entry)) || {};
        const isGemLedgerEntry = (entry: InventoryLedgerEntry): boolean =>
          cleanName(entry?.item_type || "") === "gem" || cleanName(entry?.item_category || "") === "currency";
        const recordTradeGemLedger = async (
          client: PoolClient,
          playerId: string,
        ...
        for (const entry of requesterInventory.ledgerEntries) {
          let itemTransactionResult: QueryResult<RuntimeRecord> | null = null;
          if (entry.delta > 0) {
        ...
        const instancesForTradeEntry = (playerId: string, entry: InventoryLedgerEntry) => {
  ...
  async applyVendBuyTransaction(entry: any = {}) {
    if (!this.isReady()) {
      return { ok: false, reason: "postgres_unavailable" };
    }
    ...
    try {
      return await this.withTransaction(async (client) => {
        ...
        let repairedBeforeLock: number | null = null;
        ...
        let repairedBeforeItem: number | null = null;
        ...
        let spendInstanceMovementResult: any = await this.transferTrackedItemInstances(client, {
  ...
  async issuePunishment(entry: any = {}) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    const e = toObject(entry);
  ...
  async revokePunishment(entry: any = {}) {
    if (!this.isReady()) return { ok: false, reason: "postgres_unavailable" };
    const e = toObject(entry);
  ...
  async getActivePunishments(username: unknown, options: any = {}) {
    if (!this.isReady()) return [];
    const cleanUsername = cleanName(username);
    if (cleanUsername === "") return [];
  ...
  async hasActivePunishment(username: unknown, type: unknown, options: any = {}) {
  ...
  mirrorWorldChange(entry: RuntimeRecord) {
    if (!this.isReady()) return;
    const e = toObject(entry);
  ...
  mirrorSecurityEvent(entry: RuntimeRecord) {
    if (!this.isReady()) return;
    const e = toObject(entry);
...
(PostgresStore as any).INTEGRITY_HASH_ALGORITHM = INTEGRITY_HASH_ALGORITHM;
(PostgresStore as any).applyCanonicalInventoryRowsToPlayerState = applyCanonicalInventoryRowsToPlayerState;
(PostgresStore as any).buildTransactionLedgerHash = buildTransactionLedgerHash;
