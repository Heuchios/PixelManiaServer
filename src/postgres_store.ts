...
class PostgresStore {
  ...
  async getLandfillLeaderboard(seasonKey: unknown, limit: unknown = 10) {
    if (!this.isReady()) return { ok: false, entries: [], reason: "postgres_unavailable" };
    const cleanSeasonKey = cleanName(seasonKey);
  ...
  async getLandfillPlayerScore(username: unknown, seasonKey: unknown) {
  ...
  async insertLandfillPrizeClaim(username: unknown, seasonKey: unknown, rank: unknown) {
    if (!this.isReady()) return { ok: false, inserted: false, reason: "postgres_unavailable" };
    const cleanUsername = cleanName(username);
    const cleanSeasonKey = cleanName(seasonKey);
  ...
  async deleteLandfillPrizeClaim(username: unknown, seasonKey: unknown) {
    if (!this.isReady()) return { ok: false };
    const cleanUsername = cleanName(username);
    const cleanSeasonKey = cleanName(seasonKey);
   ...
   */
  async deleteWorldState(worldName: unknown): Promise<boolean> {
    const cleanWorldName = cleanName(worldName || "");
    if (cleanWorldName === "") return false;
    if (!this.isReady()) {
