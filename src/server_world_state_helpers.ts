...
const WORLD_STATE_COMPACT_TILE_IDS_ENABLED = !["0", "false", "no", "off"].includes(
  String(process.env.WORLD_STATE_COMPACT_TILE_IDS || "true").trim().toLowerCase()
);
...
function createWorldStateHelpers(config: WorldStateHelperConfig): WorldStateHelpers {
  ...
  function normalizeBlockEntry(rawEntry: unknown): JsonRecord | null {
    ...
    if (blockRevision > 0) entry.block_revision = blockRevision;
    const placementRequestId = clampString(rawEntry.placement_request_id || "");
    if (placementRequestId !== "") entry.placement_request_id = placementRequestId;
    const placedByAccount = config.cleanAccountName(rawEntry.placed_by_account || "");
    if (placedByAccount !== "") entry.placed_by_account = placedByAccount;
  ...
  function normalizeRemovedBlockEntry(rawEntry: unknown): JsonRecord | null {
    ...
    if (mutationRequestId !== "") entry.mutation_request_id = mutationRequestId;
    return entry;
  }
  ...
  function serializeSeedForSave(seed: unknown): unknown {
    ...
    if (!Number.isFinite(plantedAt) || plantedAt <= 0) {
      const maxGrowTime = Math.max(
        1,
        Number(serialized.max_grow_time) || config.getSeedConfiguredGrowTime(serialized.seed_type),
      );
  ...
  function deserializeWorldState(worldName: unknown, data: unknown): JsonRecord {
    ...
    for (const layerName of ["foreground", "background", "removed_foreground", "removed_background"]) {
      ...
      for (const rawEntry of layer.values()) {
        ...
        state.block_revision = Math.max(
          Number(state.block_revision) || 0,
          Math.max(0, Math.trunc(Number(rawEntry.block_revision) || 0)),
        );
      }
    }
