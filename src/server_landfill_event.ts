...
//
// Entry uses the normal snapshot/ready flow, with a landfill guard at activation.
// World-index hooks track arrivals and departures immediately; polling reconciles
// the scoring roster. Started races reject re-entry, and empty used rooms retire.
//
...
interface LandfillParticipant {
  ...
  connected: boolean;
  // Set true the instant a connected participant is observed leaving this instance before it has
  // actually finished (see reconcileParticipants) -- confirmed with Hassan 2026-08-17: leaving a
  // race before it ends forfeits that spot, so Join Race must always route to a fresh instance
  // rather than reconnecting them here. Deliberately does NOT touch kilograms/joinOrder/
  // lastProgressAtMs: a forfeited participant is still ranked and still persisted at race end
  // (see buildRankings/persistSessionResults) -- this flag only revokes REJOIN eligibility, never
  // already-earned progress. See findSessionForParticipant, which is the sole reader.
  forfeited: boolean;
}
...
interface LandfillInstance {
  ...
  // Actual world-index arrivals, synchronously removed on departure.
  participantUsernames: Set<string>;
  // Players the Join Race flow has actually routed to this session. THIS is the door policy: a
  // Landfill instance may only be entered by someone requestJoinLandfillRace explicitly sent here.
  //
  // Without it, canPlayerJoinLandfillInstance admits anyone who names a live, non-full session --
  // so typing "LANDFILL_335557" into the lobby's JOIN field, or warping to a friend who is racing,
  // walks straight into someone else's race. The instance name is not a secret: it is displayed in
  // the client's own world field the moment you are routed there.
  //
  // Admission is granted at Join Race time and consumed at join_world time; it is never granted by
  // the join itself, which is what keeps the two apart.
  admittedUsernames: Set<string>;
...
function createLandfillEventSystem(deps: LandfillDeps) {
  const {
    ...
    placementBonusKilograms = [] as number[],
  ...
  function isForfeitableState(state: LandfillSessionState): boolean {
  ...
  async function createNewInstance(): Promise<LandfillInstance> {
    ...
    const instance: LandfillInstance = {
      ...
      abandoned: false,
      ready: false,
      departedUsernames: new Set<string>(),
      createdAtMs: nowMs,
      countdownEndsAtMs: 0,
      raceStartedAtMs: 0,
      raceEndsAtMs: 0,
      ...
      admittedUsernames: new Set<string>(),
  ...
  function isInstanceJoinable(instance: LandfillInstance): boolean {
    if (!instance.ready || instance.abandoned || !isJoinableState(instance.state)) return false;
    ...
    if (instance.admittedUsernames.size >= maxPlayersPerInstance) return false;
  ...
  async function resolveLandfillInstanceJoin(worldName: unknown, username: unknown) {
    const check = canPlayerJoinLandfillInstance(worldName, username);
    if (check.ok) return { ok: true, world_name: String(worldName) };
    const instance = getInstance(worldName);
    const cleanUsername = cleanAccountName(username || "");
  ...
  function recordLandfillInstanceLeave(worldName: unknown, username: unknown): void {
    const instance = getInstance(worldName);
    const cleanUsername = cleanAccountName(username || "");
    if (!instance || !cleanUsername) return;
    ...
    if (participant) {
      ...
      if (isForfeitableState(instance.state)) participant.forfeited = true;
    }
  ...
  function findOpenInstance(username: string): LandfillInstance | null {
    for (const instance of instances.values()) {
      if (!instance.departedUsernames.has(username) && isInstanceJoinable(instance)) return instance;
    }
    return null;
  }
  ...
  async function allocateLandfillRace(username: unknown): Promise<{ ok: boolean; reason?: string; world_name?: string }> {
    ...
    for (const session of instances.values()) reconcileParticipants(session, Date.now());
    if (cleanRequester !== "") {
  ...
  function findSessionForParticipant(cleanUsername: string): LandfillInstance | null {
    ...
    for (const instance of instances.values()) {
      ...
      if (!instance.ready || instance.abandoned || !isJoinableState(instance.state) || instance.departedUsernames.has(cleanUsername)) continue;
      ...
      if (existingParticipant && !existingParticipant.forfeited) return instance;
      ...
      if (instance.admittedUsernames.has(cleanUsername)) return instance;
    }
    return null;
  }
  ...
  function getPlacementBonus(_placement: number): number {
