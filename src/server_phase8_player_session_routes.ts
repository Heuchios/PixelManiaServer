...
interface Phase8PlayerSessionDeps {
  ...
  checkLandfillInstanceJoinEligibility(worldName: unknown, username: unknown): { ok: boolean; reason?: string };
  ...
  getWorldBlockRevisionForWorld(worldName: unknown): number;
  getWorldRevision(worldName: unknown): number;
  ...
  recordLandfillInstanceJoin(worldName: unknown, username: unknown): unknown;
  ...
  createWorldEntrySessionId(): string;
}
...
const WORLD_ENTRY_PROFILE_ENABLED = ["1", "true", "yes", "on"].includes(
  String(process.env.WORLD_ENTRY_PROFILE || "").trim().toLowerCase()
) && (
  process.env.NODE_ENV !== "production"
  || ["1", "true", "yes", "on"].includes(
    String(process.env.WORLD_ENTRY_PROFILE_IN_PRODUCTION || "").trim().toLowerCase()
  )
);
...
const WORLD_ENTRY_CATCHUP_MAX_NO_PROGRESS_ATTEMPTS = Math.max(
  2,
  Math.trunc(Number(process.env.WORLD_ENTRY_CATCHUP_MAX_NO_PROGRESS_ATTEMPTS) || 10)
);
const WORLD_ENTRY_CATCHUP_RETRY_MS = Math.max(
  25,
...
const WORLD_ENTRY_PROVISIONAL_TIMEOUT_MS = Math.max(
  5000,
  Math.trunc(Number(process.env.WORLD_ENTRY_PROVISIONAL_TIMEOUT_MS) || 20000)
);
...
function createServerPhase8PlayerSessionRoutes(deps: Phase8PlayerSessionDeps) {
  ...
  function clearWorldEntrySession(player: PlayerRecord): void {
  ...
  async function activateWorldEntry(
    socket: unknown,
    player: PlayerRecord,
    context: RouteContext
  ): Promise<void> {
  ...
  async function handleJoinWorld(socket: unknown, player: PlayerRecord, data: PacketRecord, context: RouteContext): Promise<void> {
    ...
    try {
      ...
      player.world_entry_block_revision = Number(
        worldStateDelivery.block_revision || deps.getWorldBlockRevisionForWorld(newWorld) || 0
      );
  ...
  async function handleWorldEntryReady(
    socket: unknown,
    player: PlayerRecord,
    data: PacketRecord,
    context: RouteContext
  ): Promise<void> {
    if (!deps.requireAuthenticated(socket, player, "finish loading a world")) return;
    ...
    const lastCatchupBlockRevision = Math.max(
      expectedBlockRevision,
      Math.trunc(Number(player.world_entry_catchup_last_client_block_revision || 0))
    );
