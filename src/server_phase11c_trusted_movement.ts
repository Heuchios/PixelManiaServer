...
interface Phase11cTrustedMovementDeps extends Record<string, any> {}

function createServerPhase11cTrustedMovement(deps: Phase11cTrustedMovementDeps) {
  const {
  ...
  const netfoxPlayerStateRegistry = new Map<string, JsonRecord>();
  const netfoxPlayerStateRegistryByProfile = new Map<string, JsonRecord>();
  const netfoxPlayerStateRegistryByPeer = new Map<string, JsonRecord>();
  const netfoxPlayerStateRegistryBySession = new Map<string, JsonRecord>();
  const phase7TrustedPositionLoggedKeys = new Set<string>();
  const phase7TrustedPositionLastLogMs = new Map<string, number>();

  function getPlayerCurrentWorldName(player: JsonRecord | null | undefined): string {
    if (!player) return "START";
    return cleanWorld(player.current_world_id || player.current_world || player.world || "START");
  }

  function sanitizeMovementMode(value: unknown, fallback: unknown = MOVEMENT_MODE_WEBSOCKET): string {
    const clean = String(value || "").trim().toUpperCase();
    if (clean === MOVEMENT_MODE_NETFOX_REAL) return MOVEMENT_MODE_NETFOX_REAL;
    if (
      clean === MOVEMENT_MODE_CUSTOM_AUTHORITATIVE
  ...
  function updatePlayerMovementModeFromPayload(
    ...
    player.movement_mode = sanitizeMovementMode(
      rawMode,
      player.movement_mode || MOVEMENT_MODE_WEBSOCKET,
    );
  }

  function isNetfoxRealMode(player: JsonRecord | null | undefined, _data: JsonRecord | null = null): boolean {
    return sanitizeMovementMode(player?.movement_mode || MOVEMENT_MODE_WEBSOCKET)
      === MOVEMENT_MODE_NETFOX_REAL;
  }

  function isCustomAuthoritativeMode(
    player: JsonRecord | null | undefined,
    _data: JsonRecord | null = null,
  ): boolean {
    return sanitizeMovementMode(player?.movement_mode || MOVEMENT_MODE_WEBSOCKET)
      === MOVEMENT_MODE_CUSTOM_AUTHORITATIVE;
  }

  function isTrustedMovementModeName(value: unknown): boolean {
  ...
  function isPlayerTrustedMovementModeAllowed(
    player: JsonRecord | null | undefined,
    movementMode: unknown,
  ): boolean {
    const cleanMode = sanitizeMovementMode(movementMode, MOVEMENT_MODE_WEBSOCKET);
    if (!isTrustedMovementModeName(cleanMode)) return false;
    if (!TRUSTED_MOVEMENT_ALLOWLIST_ENABLED) return false;
  ...
  function usesTrustedMovementPosition(player: JsonRecord | null | undefined): boolean {
    return isNetfoxRealMode(player) || isCustomAuthoritativeMode(player);
  }

  function isTrustedMovementModeEnabled(
    mode: unknown,
    player: JsonRecord | null = null,
  ): boolean {
    const cleanMode = sanitizeMovementMode(mode, MOVEMENT_MODE_WEBSOCKET);
    if (cleanMode === MOVEMENT_MODE_NETFOX_REAL) {
      if (!NETFOX_MOVEMENT_ENABLED || !NETFOX_TRUSTED_PLAYER_STATE_ENABLED) return false;
    } else if (cleanMode === MOVEMENT_MODE_CUSTOM_AUTHORITATIVE) {
      if (!CUSTOM_TRUSTED_PLAYER_STATE_ENABLED) return false;
    } else {
      return false;
    }
  ...
  function getTrustedMovementModeRejectionReason(
    player: JsonRecord | null | undefined,
    movementMode: unknown,
  ): string | false {
    const cleanMode = sanitizeMovementMode(movementMode, MOVEMENT_MODE_WEBSOCKET);
    if (!isTrustedMovementModeName(cleanMode)) return "trusted_movement_mode_disabled";
    if (
      cleanMode === MOVEMENT_MODE_NETFOX_REAL
      && (!NETFOX_MOVEMENT_ENABLED || !NETFOX_TRUSTED_PLAYER_STATE_ENABLED)
    ) {
      return "trusted_movement_mode_disabled";
    }
    if (
      cleanMode === MOVEMENT_MODE_CUSTOM_AUTHORITATIVE
      && !CUSTOM_TRUSTED_PLAYER_STATE_ENABLED
    ) {
      return "trusted_movement_mode_disabled";
    }
    if (!TRUSTED_MOVEMENT_ALLOWLIST_ENABLED) return "trusted_movement_allowlist_disabled";
    if (!(TRUSTED_MOVEMENT_ALLOWLIST instanceof Set) || TRUSTED_MOVEMENT_ALLOWLIST.size === 0) {
  ...
  function getTrustedMovementModeLabel(
    player: JsonRecord | null | undefined,
    data: JsonRecord | null = null,
  ): string {
    if (isCustomAuthoritativeMode(player, data)) return MOVEMENT_MODE_CUSTOM_AUTHORITATIVE;
    if (isNetfoxRealMode(player, data)) return MOVEMENT_MODE_NETFOX_REAL;
    return sanitizeMovementMode(player?.movement_mode || MOVEMENT_MODE_WEBSOCKET);
  }

  function getTrustedMovementSourceLabel(mode: unknown): string {
    return sanitizeMovementMode(mode) === MOVEMENT_MODE_CUSTOM_AUTHORITATIVE
      ? "custom_movement_server"
      : "netfox";
  }

  function getNetfoxStateKey(player: JsonRecord | null | undefined): string {
    return normalizeNetfoxProfileKey(player?.id || "");
  }

  function normalizeNetfoxProfileKey(value: unknown): string {
    return String(value || "").trim();
  }

  function normalizeNetfoxPeerKey(value: unknown): string {
    const peerId = Math.max(0, Math.trunc(Number(value) || 0));
    return peerId > 0 ? String(peerId) : "";
  }

  function normalizeOptionalNetfoxWorld(value: unknown): string {
    const raw = String(value || "").trim();
    return raw === "" ? "" : cleanWorld(raw);
  }

  function clearNetfoxTrustedPlayerStateByKey(key: unknown): void {
    ...
    const profileKey = normalizeNetfoxProfileKey(
      existingState.game_player_id || existingState.player_id || cleanKey,
    );
    if (profileKey !== "") {
  ...
  function clearNetfoxTrustedPlayerState(player: JsonRecord | null | undefined): void {
    clearNetfoxTrustedPlayerStateByKey(getNetfoxStateKey(player));
  }

  function logPhase7TrustedPosition(state: JsonRecord | null, reason = "update"): void {
    if (!state) return;

    const profileKey = normalizeNetfoxProfileKey(
      state.game_player_id || state.player_id || state.state_key || "",
    );
    ...
    logger.log(
      `[Phase7TrustedPosition] profile=${profileKey} peer=${Number(peerKey)} `
  ...
  function indexNetfoxTrustedPlayerState(
    player: JsonRecord | null | undefined,
    state: JsonRecord,
  ): void {
    ...
    const profileKey = normalizeNetfoxProfileKey(
      state.game_player_id || state.player_id || stateKey,
    );
    const peerKey = normalizeNetfoxPeerKey(state.peer_id);
    ...
    if (sessionKey !== "") netfoxPlayerStateRegistryBySession.set(sessionKey, indexedState);
    logPhase7TrustedPosition(indexedState, "update");
  }

  function debugNetfoxAction(message: string, details: JsonRecord = {}): void {
    if (!NETFOX_ACTION_DEBUG) return;
    logger.log("[NETFOX_ACTION]", message, details);
  }

  function enforceStandardMovementForSocket(
    socket: any,
    player: JsonRecord,
    context = "movement",
  ): boolean {
    const cleanMode = getTrustedMovementModeLabel(player);
    if (!isTrustedMovementModeName(cleanMode)) return true;
    if (isTrustedMovementModeEnabled(cleanMode, player)) return false;
    ...
    sendActionRejected(
      socket,
      "movement_mode",
      "That movement mode is no longer enabled on this server.",
      {
        reason,
        previous_mode: cleanMode,
        action: context,
  ...
  function rejectTrustedMovementMode(
    socket: any,
    player: JsonRecord,
    movementMode: unknown,
    authAction: string,
    ...
    sendActionRejected(
      socket,
      authAction,
      "That movement mode is not enabled on this server.",
      { reason, movement_mode: cleanMode },
    );
  }

  function clearTrustedMovementBaseline(player: JsonRecord | null | undefined): void {
    if (!player) return;
    player.trusted_movement_baseline = null;
  }

  function getTrustedMovementBaseline(
    player: JsonRecord | null | undefined,
  ): JsonRecord | null {
    ...
    if (![x, y, updatedAt].every(Number.isFinite) || updatedAt <= 0) return null;
    ...
    return {
      ...
      movement_mode: sanitizeMovementMode(
        baseline.movement_mode || player?.movement_mode || MOVEMENT_MODE_WEBSOCKET,
      ),
    };
  }

  function commitTrustedMovementBaseline(
    player: JsonRecord | null | undefined,
    ...
    const movementMode = sanitizeMovementMode(
      details.movement_mode
  ...
  function clearPlayerWorldEntrySpawnGuard(player: JsonRecord | null | undefined): void {
    if (!player) return;
    delete player.world_entry_spawn_guard;
  }

  function setPlayerWorldEntrySpawnGuard(
    player: JsonRecord | null | undefined,
    worldName: unknown,
    spawn: JsonRecord | null,
    now = nowMs(),
  ): JsonRecord | null {
    ...
    if (world === "" || !isPositionInWorldBounds(x, y)) return null;
  ...
  function getPlayerWorldEntrySpawnGuard(
    player: JsonRecord | null | undefined,
    worldName: unknown,
    now = nowMs(),
  ): JsonRecord | null {
  ...
  function checkPlayerWorldEntrySpawnGuard(
    player: JsonRecord | null | undefined,
    position: JsonRecord | null,
    now = nowMs(),
  ): JsonRecord {
    const guard = getPlayerWorldEntrySpawnGuard(player, position?.world, now);
    if (!guard) return { active: false, accepted: false, distance: null, guard: null };
  ...
  function getTrustedMovementBaselineResetReason(
    player: JsonRecord,
    position: JsonRecord,
    data: JsonRecord,
    movementMode: string,
    ...
    if (isAdmin(player) && player.noclip_enabled) return "admin_noclip";
    if (isValidRespawnTeleportPosition(player, position, data)) return "respawn_teleport";
    if (!baseline) return "first_state";
    ...
    if (baselineWorld !== worldName) return "world_changed";

    const baselineMode = sanitizeMovementMode(
      baseline.movement_mode || player?.movement_mode || MOVEMENT_MODE_WEBSOCKET,
    );
    if (baselineMode !== movementMode) return "mode_changed";

    const baselinePeerId = Math.max(0, Math.trunc(Number(baseline.peer_id) || 0));
    if (peerId > 0 && baselinePeerId > 0 && peerId !== baselinePeerId) return "peer_changed";

    const baselineTick = Math.max(0, Math.trunc(Number(baseline.tick) || 0));
    if (tick > 0 && baselineTick > 0 && tick < baselineTick) return "tick_reset";
  ...
  function acceptTrustedMovementState(
    ...
    const peerId = Math.max(
      0,
      Math.trunc(Number(options.peer_id ?? options.peerId ?? data?.peer_id) || 0),
    );
    ...
    const resetReason = getTrustedMovementBaselineResetReason(
      player,
      position,
      data,
      cleanMovementMode,
      baseline,
      { now, peer_id: peerId, tick },
    );
    if (resetReason !== "") {
      commitTrustedMovementBaseline(player, position, {
        now,
    ...
    const distance = Math.hypot(
      Number(position.x) - Number(baseline?.x),
      Number(position.y) - Number(baseline?.y),
    );
    const velocitySpeed = Math.hypot(
      sanitizePlayerVelocity(data?.velocity_x),
      sanitizePlayerVelocity(data?.velocity_y),
    );
    ...
    const trustedCollision = getMovementCollisionAtPosition(
  ...
  function validateNetfoxTrustedPositionState(
    ...
    const maxAgeMs = Math.max(
      50,
      Math.trunc(Number(getTrustedPositionMaxAgeMs(action)) || MAX_TRUSTED_POSITION_AGE_MS),
    );
    ...
    return {
      ...
      game_player_id: normalizeNetfoxProfileKey(
        state.game_player_id || state.player_id || state.state_key || "",
      ),
  ...
  function get_trusted_position_for_profile(
    gamePlayerId: unknown,
    options: JsonRecord = {},
  ): JsonRecord {
    ...
    return validateNetfoxTrustedPositionState(
      netfoxPlayerStateRegistryByProfile.get(profileKey),
      options,
    );
  }

  function get_trusted_position_for_peer(
    peerId: unknown,
    options: JsonRecord = {},
  ): JsonRecord {
    ...
    return validateNetfoxTrustedPositionState(
      netfoxPlayerStateRegistryByPeer.get(peerKey),
      options,
    );
  }

  function get_trusted_position_for_session(
    sessionId: unknown,
    options: JsonRecord = {},
  ): JsonRecord {
    ...
    return validateNetfoxTrustedPositionState(
      netfoxPlayerStateRegistryBySession.get(sessionKey),
      options,
    );
  }

  function get_netfox_peer_for_profile(
    gamePlayerId: unknown,
    options: JsonRecord = {},
  ): JsonRecord {
  ...
  function getNetfoxTrustedPlayerState(
    player: JsonRecord | null | undefined,
    options: JsonRecord = {},
  ): JsonRecord {
  ...
  function getPlayerValidationPosition(
    player: JsonRecord | null | undefined,
    options: JsonRecord = {},
  ): JsonRecord {
  ...
  function rejectMissingTrustedPosition(
    socket: any,
    action: string,
    player: JsonRecord,
    result: JsonRecord,
    ...
    const world = cleanWorld(
      extra.world || result?.expected_world || getPlayerCurrentWorldName(player),
    );
    ...
    if (reason !== "missing") {
      logSecurityEvent(
        socket,
        player,
        isCustom ? "custom_trusted_position_reject" : "netfox_trusted_position_reject",
        {
  ...
  function validateNetfoxActionCooldown(
    socket: any,
    player: JsonRecord,
    action: string,
    data: JsonRecord | null = null,
  ): boolean {
    if (!usesTrustedMovementPosition(player)) return true;
  ...
  function handleTrustedMovementPlayerStateClear(
    socket: any,
    player: JsonRecord,
    data: JsonRecord,
    movementMode: unknown,
    ...
    if (!requireAuthenticated(socket, player, authAction)) return;

    const cleanMovementMode = sanitizeMovementMode(movementMode, MOVEMENT_MODE_WEBSOCKET);
    if (!isTrustedMovementModeName(cleanMovementMode)) return;
    ...
    if (TRUSTED_MOVEMENT_REQUIRE_JOINED_WORLD && !player.joined_world) {
      rejectTrustedMovementMode(
        ...
        cleanMovementMode,
        authAction,
        "trusted_movement_world_not_joined",
      );
      return;
    }
    ...
    const worldName = cleanWorld(
      data.world || oldState?.world || getPlayerCurrentWorldName(player),
    );
    const reason = normalizePhase7Reason(data.reason || "clear");
    logger.log(
      `[PhaseI] trusted_position_cleared `
      + `username=${cleanAccountName(player?.account_username || player?.name || "")} `
  ...
  function handleTrustedMovementPlayerState(
    socket: any,
    player: JsonRecord,
    data: JsonRecord,
    movementMode: unknown,
    ...
    if (!requireSameWorld(socket, player, position.world, authAction)) return;

    const peerId = Math.max(0, Math.trunc(Number(data.peer_id) || 0));
    const tick = Math.max(0, Math.trunc(Number(data.tick) || 0));
    const movementCheck = acceptTrustedMovementState(
      socket,
      player,
      position,
      data,
    ...
    const playerNodePath = clampString(
      data.player_node_path || data.node_path || data.custom_player_node_path || "",
    );
    const state = {
  ...
  function handleNetfoxTrustedPlayerState(
    socket: any,
    player: JsonRecord,
    data: JsonRecord,
  ): void {
    handleTrustedMovementPlayerState(
      ...
      data,
      MOVEMENT_MODE_NETFOX_REAL,
      "sync Netfox movement",
    );
  }

  function handleCustomTrustedPlayerState(
    socket: any,
    player: JsonRecord,
    data: JsonRecord,
  ): void {
    handleTrustedMovementPlayerState(
      ...
      data,
      MOVEMENT_MODE_CUSTOM_AUTHORITATIVE,
      "sync custom movement",
    );
  }

  function handleCustomTrustedPlayerStateClear(
    socket: any,
    player: JsonRecord,
    data: JsonRecord,
  ): void {
    handleTrustedMovementPlayerStateClear(
      ...
      data,
      MOVEMENT_MODE_CUSTOM_AUTHORITATIVE,
      "clear custom movement",
    );
  }

  function applyTrustedActionPositionFromPayload(
    socket: any,
    player: JsonRecord,
    data: JsonRecord,
    fallbackWorld = "",
    ...
    if (getTrustedMovementModeRejectionReason(player, movementMode)) return false;
    if (TRUSTED_MOVEMENT_REQUIRE_JOINED_WORLD && !player.joined_world) return false;

    const position = sanitizeActionPositionPayload(data, player, fallbackWorld);
    if (!position) return false;

    const peerId = Math.max(0, Math.trunc(Number(
      data.actor_peer_id
      ?? data.player_peer_id
      ?? data.peer_id
    ...
    const tick = Math.max(
      0,
      Math.trunc(Number(data.actor_tick ?? data.player_tick ?? data.tick ?? 0) || 0),
    );
    const movementData = {
      ...data,
  ...
  function resetPlayerMovementTracking(
    player: JsonRecord | null | undefined,
    now = nowMs(),
  ): void {
    if (!player) return;
  ...
  function getRegistryStats(): JsonRecord {
    return {
