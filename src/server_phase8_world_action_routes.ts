...
interface Phase8WorldActionDeps extends Record<string, any> {}

const WORLD_BLOCK_PLACE_INVENTORY_LOCK_WAIT_MS = 650;
const WORLD_BLOCK_PLACE_INVENTORY_LOCK_RETRY_MS = 25;

function createServerPhase8WorldActionRoutes(deps: Phase8WorldActionDeps) {
  const {
  ...
  async function acquirePlayerInventoryLocksWithWait(
    usernames: string[],
    owner: string,
    maxWaitMs: number = WORLD_BLOCK_PLACE_INVENTORY_LOCK_WAIT_MS,
    retryMs: number = WORLD_BLOCK_PLACE_INVENTORY_LOCK_RETRY_MS
  ): Promise<any> {
  ...
  function buildBlockUpdateRejectionDetails(worldName: string, update: PacketRecord, requestId: string, extra: PacketRecord = {}): PacketRecord {
    const x = Math.trunc(Number(update?.x) || 0);
    const y = Math.trunc(Number(update?.y) || 0);
    const details: PacketRecord = {
  ...
  async function handleWorldBlockUpdate(socket: any, player: any, data: PacketRecord, context: RouteContext): Promise<void> {
    let blockActionLock: any = null;
          ...
          const releasePlacementInventoryLock = () => {
            if (!placementInventoryLock) return;
          ...
          const releaseUniqueSpecialBlockPlacementLock = () => {
            if (!uniqueSpecialBlockPlacementLock) return;
          ...
          if (update.action === "place" && uniqueSpecialBlockPlacementResource !== "") {
            uniqueSpecialBlockPlacementLock = await acquireLiveActionLock(
              worldSpecialBlockActionLocks,
              "unique_special_block_place",
              uniqueSpecialBlockPlacementResource,
              player.id
            );
          ...
          if (update.action === "place" && isPostgresAuthoritativeReady()) {
            ...
            if (placementCostForLock && Number(placementCostForLock.amount) > 0) {
              ...
              placementInventoryLock = await acquirePlayerInventoryLocksWithWait(
                [inventoryOwner],
                `world_block_place:${worldName}:${requestId || blockActionResource}`
              );
          ...
          if (electricalGeneration && electricalGeneration.generator) {
            const refreshedGenerator = getMutableElectricalDeviceStateAt(
              ensureWorldState(worldName),
              electricalGeneration.generator.x,
              electricalGeneration.generator.y
            );
          ...
          if (areaLockStatePayload) {
            ...
            placementInteractionChanges.push(buildWorldObjectChangeEntry(
              ...
              player,
              worldName,
              areaLockStatePayload,
              areaLockStateBefore,
              getAreaLocksJournalData(worldName),
              blockTransactionId,
              {
          ...
          const dropWorldChangeEntries = emittedDrops.map((drop: any) => ({
            ...getAuditActor(socket, player),
          ...
          if (electricalGeneration && Number(electricalGeneration.generated_watts || 0) > 0 && electricalGeneration.generator) {
            ...
            electricalGenerationChanges.push(buildWorldObjectChangeEntry(
              socket,
              player,
              worldName,
              {
              ...
              electricalGeneration.generator_before || {},
              generator,
              blockTransactionId,
              {
                source_block: update.block_type,
  ...
  async function handleElectricalLayerUpdate(socket: any, player: any, data: PacketRecord, context: RouteContext): Promise<void> {
    try {
            if (!requireAuthenticated(socket, player, "edit wiring")) return;

            const worldName = getPlayerCurrentWorldName(player);
            if (await rejectIfWorldBanned(socket, player, worldName, "electrical_layer_update")) return;
            const allowDevJsonFallback = shouldAllowPhase7DevJsonFallback(player, data, {
              world: worldName,
              allow_dev_json_fallback: true,
            });
            ...
            const electricalChangeEntry = buildWorldObjectChangeEntry(
              socket,
              player,
              worldName,
              objectUpdate,
  ...
  async function handleRequestWireVisibilityRefresh(socket: any, player: any, data: PacketRecord, context: RouteContext): Promise<void> {
    if (!requireAuthenticated(socket, player, "refresh wiring visibility")) return;

          const worldName = cleanWorld(data.world || player.world || "START");
          if (!requireSameWorld(socket, player, worldName, "refresh wiring visibility there")) return;
          if (await rejectIfWorldBanned(socket, player, worldName, "request_wire_visibility_refresh")) return;
  ...
  async function handleRequestOpenGenerator(socket: any, player: any, data: PacketRecord, context: RouteContext): Promise<void> {
    if (!requireAuthenticated(socket, player, "open transformers")) return;

          const worldName = cleanWorld(data.world || player.world || "START");
          if (!requireSameWorld(socket, player, worldName, "open that transformer")) return;
          if (await rejectIfWorldBanned(socket, player, worldName, "request_open_generator")) return;
          ...
          if (payload) sendJson(socket, payload);
          return;
  }
  ...
  async function handleRequestLinkGeneratorPad(socket: any, player: any, data: PacketRecord, context: RouteContext): Promise<void> {
    if (!requireAuthenticated(socket, player, "link transformer circuits")) return;
          ...
          const worldName = cleanWorld(data.world || player.world || "START");
          if (!requireSameWorld(socket, player, worldName, "link that transformer")) return;
          if (await rejectIfWorldBanned(socket, player, worldName, "request_link_generator_pad")) return;
          ...
          if (!Number.isFinite(generatorX) || !Number.isFinite(generatorY) || !Number.isFinite(padX) || !Number.isFinite(padY)) return;
          ...
          if (!isGridInWorld(gridGeneratorX, gridGeneratorY) || !isGridInWorld(gridPadX, gridPadY)) return;
          ...
          if (linkedPadKeys.includes(padKey)) {
            ...
            if (alreadyPayload) sendJson(socket, alreadyPayload);
            return;
          }
          ...
          const linkChangeEntry = buildWorldObjectChangeEntry(
            socket,
            player,
            worldName,
            {
          ...
          if (linkPayload) sendElectricalPayloadToVisiblePlayers(worldName, linkPayload);
  ...
  async function handleRequestLinkGeneratorPole(socket: any, player: any, data: PacketRecord, context: RouteContext): Promise<void> {
    if (!requireAuthenticated(socket, player, "link transformer outputs")) return;
          ...
          const worldName = cleanWorld(data.world || player.world || "START");
          if (!requireSameWorld(socket, player, worldName, "link that transformer output")) return;
          if (await rejectIfWorldBanned(socket, player, worldName, "request_link_generator_pole")) return;

          const generatorX = Number(data.generator_x);
          const generatorY = Number(data.generator_y);
          const poleX = Number(data.pole_x);
          const poleY = Number(data.pole_y);
          if (!Number.isFinite(generatorX) || !Number.isFinite(generatorY) || !Number.isFinite(poleX) || !Number.isFinite(poleY)) return;
          const gridGeneratorX = Math.trunc(generatorX);
          const gridGeneratorY = Math.trunc(generatorY);
          const gridPoleX = Math.trunc(poleX);
          const gridPoleY = Math.trunc(poleY);
          if (!isGridInWorld(gridGeneratorX, gridGeneratorY) || !isGridInWorld(gridPoleX, gridPoleY)) return;
  ...
  async function handleRequestLinkElectricPoles(socket: any, player: any, data: PacketRecord, context: RouteContext): Promise<void> {
    if (!requireAuthenticated(socket, player, "link electric poles")) return;
          ...
          const worldName = cleanWorld(data.world || player.world || "START");
          if (!requireSameWorld(socket, player, worldName, "link those electric poles")) return;
          if (await rejectIfWorldBanned(socket, player, worldName, "request_link_electric_poles")) return;
          ...
          if (!Number.isFinite(poleAX) || !Number.isFinite(poleAY) || !Number.isFinite(poleBX) || !Number.isFinite(poleBY)) return;
          ...
          if (!isGridInWorld(gridPoleAX, gridPoleAY) || !isGridInWorld(gridPoleBX, gridPoleBY)) return;
