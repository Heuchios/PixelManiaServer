import * as Market from "./server_fish_market";
import ItemDatabase = require("./server_item_database");
import PostgresContracts = require("./postgres_store_contracts");

const localRows = new Map<string, Market.MarketRow>();

export async function ensureSchema(store: any): Promise<void> {
  ...
  try {
    ...
    if (!migrated.rowCount) {
      ...
      const isFish = (id: string) => ItemDatabase.getItemDefinition(id)?.category === "fish";
      ...
      for (const row of worlds.rows) {
        ...
        if (!Market.migrateWorldHoldings(row.world_state, isFish)) continue;
        const checksumState = { ...row.world_state };
        delete checksumState.saved_at;
        delete checksumState.last_saved_at;
        await client.query(`UPDATE ${store.table("worlds")} SET world_state = $2::jsonb, world_checksum = $3 WHERE world_id = $1`,
          [row.world_id, JSON.stringify(row.world_state), PostgresContracts.jsonChecksum(checksumState)]);
...
export async function quotes(store: any, ids: string[], now = Date.now()): Promise<Record<string, Market.Quote>> {
  const result: Record<string, Market.Quote> = {};
  const stored = new Map<string, Market.MarketRow>();
  if (store && ids.length) {
    ...
    for (const row of query.rows) stored.set(row.item_id, { ...row, revision: Number(row.revision), updated_ms: Number(row.updated_ms) });
  }
  for (const id of [...new Set(ids)].sort()) {
    const definition = ItemDatabase.getItemDefinition(id);
    if (!definition || definition.category !== "fish" || definition.hidden) continue;
    const settings = Market.policy(definition);
    let row: Market.MarketRow;
    if (store) {
      row = stored.get(id)!;
    } else {
...
export async function lockSale(store: any, client: any, metadata: any, deltas: any[], playerId = "", requestId = ""): Promise<void> {
  if (playerId && requestId) {
    ...
    if (!receipt.rowCount) throw new Error("This fish sale was already completed.");
  }
...
export function commitLocal(lines: Array<Market.Quote & { amount: number }>): void {
  for (const line of lines) localRows.set(line.item_id, {
