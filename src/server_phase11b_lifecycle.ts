...
interface Phase11bLifecycleDeps extends Record<string, any> {}

interface FlushPendingSavesOptions {
  syncLocalJson?: boolean;
}
...
function createServerPhase11bLifecycle(deps: Phase11bLifecycleDeps) {
  const {
    ACCOUNTS_SAVE_PATH,
    ADMIN_LOG_PATH,
  ...
  let antiDupeAuditTimer: TimerHandle = null;
  let antiDupeAuditStartupTimer: TimerHandle = null;
  let antiDupeAuditRunning = false;
  let worldSnapshotSchedulerTimer: TimerHandle = null;
  let worldSnapshotSchedulerStartupTimer: TimerHandle = null;
  ...
  function clearIntervalTimer(timer: TimerHandle): null {
    if (timer) timerApi.clearInterval(timer);
    return null;
  }

  function clearTimeoutTimer(timer: TimerHandle): null {
    if (timer) timerApi.clearTimeout(timer);
    return null;
  }

  function migrateLegacyDataFolders(): void {
    for (const legacyFolder of LEGACY_DATA_FOLDERS as string[]) {
      if (!fileSystem.existsSync(legacyFolder)) continue;

      persistenceHelpers.copyJsonIfMissingOrNewer(
        pathModule.join(legacyFolder, "accounts.json"),
        ACCOUNTS_SAVE_PATH,
        "accounts file",
      );
      persistenceHelpers.copyJsonFolderIfMissingOrNewer(
        pathModule.join(legacyFolder, "worlds"),
        WORLD_SAVE_FOLDER,
        "worlds",
      );
      persistenceHelpers.copyJsonFolderIfMissingOrNewer(
        pathModule.join(legacyFolder, "players"),
        PLAYER_SAVE_FOLDER,
        "players",
      );
    }
  ...
  function ensureDataFolders(): void {
  ...
  function getWorldSavePath(worldName: unknown): string {
    return pathModule.join(
      WORLD_SAVE_FOLDER,
      `${safeFileName(cleanWorld(worldName), "START")}.json`,
    );
  }

  function getPlayerSavePath(username: unknown): string {
    return pathModule.join(
      PLAYER_SAVE_FOLDER,
      `${safeFileName(cleanAccountName(username).toLowerCase(), "guest")}.json`,
    );
  }
  ...
  function listJsonFiles(folder: string): string[] {
    try {
      ...
      return (fileSystem.readdirSync(folder, { withFileTypes: true }) as any[])
  ...
  function readPlayerStatesFromJsonFolder(): JsonRecord[] {
    ...
    for (const filePath of listJsonFiles(PLAYER_SAVE_FOLDER)) {
      ...
      const state = sanitizePlayerState(
        ...
        record.username || fallbackUsername,
      );
      if (!state) continue;
  ...
  function loadPlayerStatesFromJsonFolder(): number {
    let loaded = 0;
  ...
  function readWorldStatesFromJsonFolder(): Array<{ worldName: string; state: JsonRecord }> {
    const states: Array<{ worldName: string; state: JsonRecord }> = [];
  ...
  function loadWorldStatesFromJsonFolder(): number {
    let loaded = 0;
  ...
  async function loadPersistentState(): Promise<void> {
    loadAccountsFromJson();
    const jsonAccounts = new Map<string, JsonRecord>(accounts as Map<string, JsonRecord>);
    ...
    if (dbAccounts.length > 0) {
      ...
      const missingAccounts: JsonRecord[] = [];
    ...
    if (dbPlayers.length > 0) {
      ...
      const missingPlayers: Array<{ username: string; state: JsonRecord }> = [];
      for (const state of readPlayerStatesFromJsonFolder()) {
      ...
      if (importedPlayers > 0) {
        await postgresStore.savePlayerStates(
          Array.from((playerStates as Map<string, JsonRecord>).values()).map((state) => ({
  ...
  async function runAntiDupeAuditNow(): Promise<void> {
  ...
  function startAntiDupeAuditScanner(): void {
  ...
  function selectWorldsForSnapshotCycle(loadedWorldNames: unknown): string[] {
    const worlds = Array.isArray(loadedWorldNames)
      ? loadedWorldNames.filter(Boolean) as string[]
      : [];
  ...
  async function runWorldSnapshotCycleNow(): Promise<void> {
  ...
  function startPeriodicWorldSnapshotScheduler(): void {
    ...
    worldSnapshotSchedulerTimer = timerApi.setInterval(() => {
      runWorldSnapshotCycleNow().catch((error: unknown) => {
        worldSnapshotSchedulerState.last_error = cleanText(getErrorMessage(error) || "unknown");
  ...
  function getWorldSnapshotSchedulerRunning(): boolean {
    return worldSnapshotSchedulerRunning;
  }

  function flushPendingSaves(options: FlushPendingSavesOptions = {}): void {
  ...
  async function shutdown(signal = ""): Promise<void> {
  ...
  function installShutdownHandlers(): void {
    ...
    processRuntime.on("exit", (code: unknown) => {
  ...
  function getLifecycleState(): JsonRecord {
    return {
