...
interface Phase11dStandardMovementDeps extends Record<string, any> {}

function createServerPhase11dStandardMovement(deps: Phase11dStandardMovementDeps) {
  const {
  ...
  const MOVEMENT_ANOMALY_LOG_INTERVAL_MS = Math.max(
    250,
    Math.trunc(Number(deps.MOVEMENT_ANOMALY_LOG_INTERVAL_MS) || 2000),
  );
  // Global ceiling on top of the per-player interval so a world-wide incident
  // cannot flood production logs.
  const MOVEMENT_ANOMALY_LOG_MAX_PER_MINUTE = Math.max(
    1,
    Math.trunc(Number(deps.MOVEMENT_ANOMALY_LOG_MAX_PER_MINUTE) || 60),
  );
  const logMovementAnomaly = typeof deps.logMovementAnomaly === "function"
    ? deps.logMovementAnomaly
    : (label: string, details: JsonRecord) => {
  ...
  function reportMovementAnomaly(
    ...
    logMovementAnomaly(label, {
      ...
      last_accepted_client_time_msec: Math.max(
        0,
        Math.trunc(Number(player.movement_client_time_msec) || 0),
      ),
   ...
   */
  function clampMovementTowardAuthoritative(
    player: JsonRecord,
    position: JsonRecord,
    maxDistance: number,
    distance: number,
    ...
    if (!isPositionInWorldBounds(clampedX, clampedY)) return null;

    const clampedPosition: JsonRecord = { ...position, x: clampedX, y: clampedY };
    // Never advance the authority through solid geometry, even by a partial step.
    if (getMovementCollisionAtPosition(
  ...
  function commitAcceptedMovementTiming(
    ...
    if (sequence > 0) player.movement_sequence = sequence;
    if (clientTimeMsec > 0) player.movement_client_time_msec = clientTimeMsec;
  }

  function sanitizePlayerVelocity(value: unknown): number {
    const velocity = Number(value);
    if (!Number.isFinite(velocity)) return 0;
    return Math.max(-2000, Math.min(2000, velocity));
  }

  function sanitizePlayerAnimationState(value: unknown): string {
  ...
  function sanitizePlayerPosition(
    data: JsonRecord | null | undefined,
    player: JsonRecord | null | undefined,
  ): JsonRecord | null {
  ...
  function sanitizePlayerDamageFlash(data: JsonRecord | null | undefined): JsonRecord {
  ...
  function getPublicPlayerDamageFlash(player: JsonRecord | null | undefined): JsonRecord {
    const expiresAt = Number(player?.damage_flash_expires_at || 0);
    const remainingMs = Math.max(
      0,
      Math.min(MAX_DAMAGE_FLASH_MS, Math.trunc(expiresAt - nowMs())),
    );
    return {
  ...
  function clearPlayerFishingPresence(player: JsonRecord | null | undefined): void {
    if (!player) return;
  ...
  function applyPlayerFishingPresenceFromSession(
    player: JsonRecord | null | undefined,
  ...
  function refreshPlayerFishingPresence(
    player: JsonRecord | null | undefined,
    worldName: unknown = "",
  ): boolean {
  ...
  function isValidRespawnTeleportPosition(
    player: JsonRecord | null | undefined,
    position: JsonRecord | null | undefined,
    data: JsonRecord | null | undefined,
  ): boolean {
    const reason = clampString(data?.position_reason || "").toLowerCase();
    if (data?.respawn_teleport !== true && reason !== "respawn") return false;

    const worldName = cleanWorld(position?.world || player?.world || "");
    if (worldName === "" || worldName !== cleanWorld(player?.world || "")) return false;
    ...
    const distance = Math.hypot(
      Number(position?.x) - Number(spawn.x),
      Number(position?.y) - Number(spawn.y),
    );
    if (Number.isFinite(distance) && distance <= TILE_SIZE * 1.5) return true;
    ...
    if (!block || !isCheckpointBlockType(block.block_type)) return false;

    const checkpointSpawn = getGridCenterPixels(checkpointX, checkpointY);
    const checkpointDistance = Math.hypot(
      Number(position?.x) - Number(checkpointSpawn.x),
      Number(position?.y) - Number(checkpointSpawn.y),
    );
    return Number.isFinite(checkpointDistance)
      && checkpointDistance <= TILE_SIZE * 1.5;
  }

  function sendPlayerPositionCorrection(
    socket: unknown,
    player: JsonRecord | null | undefined,
    position: JsonRecord | null | undefined,
    reason: string,
    ...
    sendActionRejected(socket, "player_position", "Server corrected your position.", {
      ...
      accepted_client_time_msec: Math.max(
        0,
        Math.trunc(Number(player.movement_client_time_msec) || 0),
      ),
  ...
  function acceptPlayerMovement(
    ...
    if (
      Object.prototype.hasOwnProperty.call(data, "velocity_x")
      ...
      const velocityDelta = Math.hypot(
        reportedVelocityX - previousVelocityX,
        reportedVelocityY - previousVelocityY,
      );
    ...
    const collision = getMovementCollisionAtPosition(
  ...
  function buildPublicPlayerPresencePayload(
    ...
    return {
      ...
      server_time_msec: Math.max(
        0,
        Math.trunc(Number(player?.movement_server_time_msec) || now),
      ),
  ...
  function getPlayerPresenceBroadcastSignature(payload: JsonRecord = {}): string {
    ...
    return JSON.stringify({
      ...
      damage_flash_remaining_ms: clampInteger(
        payload.damage_flash_remaining_ms || 0,
        0,
        MAX_DAMAGE_FLASH_MS,
      ),
