...
function createServerAccountSessionHelpers(deps: AccountSessionDeps) {
  ...
  async function makePasswordHashAsync(
    password: unknown,
  ...
  async function verifyPasswordAsync(
    account: PacketRecord | null | undefined,
    password: unknown,
  ): Promise<boolean> {
    if (!account || !account.password_salt || !account.password_hash) return false;

    const result = await makePasswordHashAsync(
      password,
      account.password_salt,
      account.password_algorithm || "legacy_scrypt",
    );
