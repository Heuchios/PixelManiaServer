...
type RouteHandler = (socket: unknown, player: PacketRecord, data: PacketRecord) => MaybePromise<unknown>;

interface InventoryEconomyRoutesDeps extends Record<string, any> {}

function createServerInventoryEconomyRoutes(deps: InventoryEconomyRoutesDeps) {
  const {
  ...
  const delegatedInventoryActions = new Map<string, RouteHandler>([
    ...
    ["donation_box_get_state", handleDonationBoxTransaction],
    ["donation_box_donate", handleDonationBoxTransaction],
    ["donation_box_retrieve", handleDonationBoxTransaction],
    ["donation_box_retrieve_all", handleDonationBoxTransaction],
  ...
  function getPackRewardTable(itemId: string): PacketRecord[] | null {
    if (itemId === "lure_pack") return LURE_PACK_TABLE;
    if (itemId === "basic_items_pack") return BASIC_ITEMS_PACK_TABLE;
    if (itemId === "hairpack") return HAIR_PACK_TABLE;
    if (itemId === "prestige_coloured_block_pack") return PRESTIGE_COLOURED_BLOCK_PACK_TABLE;
    return null;
  }

  function getShopPurchaseMessage(itemId: string, listing: PacketRecord): string {
    if (itemId === "lure_pack") return "Purchased and opened Lure Pack.";
    if (itemId === "basic_items_pack") return "Purchased and opened Basic Items Pack.";
    if (itemId === "hairpack") return "Purchased and opened Hair Pack.";
    if (itemId === "prestige_coloured_block_pack") return "Purchased and opened Prestige Coloured Block Pack.";
    return `Purchased ${listing.item_id}.`;
  }

  function getShopItemDisplayName(itemId: string): string {
  ...
  function getFullPackReward(state: PacketRecord, rewardTable: PacketRecord[]): PacketRecord | null {
    const checkedRewards = new Set<string>();
    for (const reward of rewardTable) {
      const rewardKey = `${reward.item_category}:${reward.item_id}`;
      if (checkedRewards.has(rewardKey)) continue;
  ...
  async function handleInventoryTransactionRequest(
    socket: unknown,
    player: PacketRecord,
    data: PacketRecord,
  ): Promise<void> {
  ...
  async function handleInventoryUpgradePurchase(
    socket: unknown,
    player: PacketRecord,
    data: PacketRecord = {},
  ): Promise<void> {
    if (!requireAuthenticated(socket, player, "upgrade inventory")) return;

    const requestId = makeRequestId(data);
    if (tradeByPlayerId.has(player?.id || (socket as PacketRecord)?.playerId || "")) {
    ...
    logItemLedgerForState(
      ...
      "inventory_upgrade",
      purchaseId,
      "inventory_slot_upgrade",
      player.world,
      { current_slots: currentSlots, next_slots: committedState.inventory_slot_count },
  ...
  async function handleShopBuyTransaction(
    socket: unknown,
    player: PacketRecord,
    data: PacketRecord,
  ): Promise<void> {
    ...
    if (
      !ItemDatabase.hasItem(listing.item_id)
    ...
    if (packRewardTable) {
      const rewardTableValid = packRewardTable.every((reward: PacketRecord) => (
    ...
    if (packRewardTable) {
      ...
      if (fullReward) {
        ...
        sendInventoryTransactionRejected(
          socket,
          data,
          `${displayName} is already at full stack. Drop or clear some before buying this pack.`,
        );
        return;
      }
    }
    ...
    if (packRewardTable) {
      ...
      if (
        !canAddItemToState(stagedState, listing.item_id, listing.item_category, listing.amount)
