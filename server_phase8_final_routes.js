...
function createServerPhase8FinalRoutes(deps) {
    const { DropContracts, InventoryContracts, acceptPlayerMovement, accountKey, accounts, acquireLiveActionLock, acquirePlayerInventoryLocks, applyDropCreateToWorldState, applyDropPickupToWorldState, applyDropPickupWorldState, applyDropUpdateToWorldState, applyInteractionUpdateToWorldState, buildInventoryDeltaClientPayloads, buildPublicPlayerPresencePayload, buildWorldInteractionDetails, buildWorldObjectChangeEntry, clampInteger, cleanAccountName, cleanName, cleanWorld, cloneJson, commitPlayerInventoryState, commitWorldStateWithBlockChanges, debugActionPositionFlow, debugNetfoxAction, deserializeWorldState, enforceStandardMovementForSocket, ensureWorldState, ensureWritablePlayerState, getEquipmentSlotsComparisonKey, getActiveThemeMachineDisableUpdates, getInventoryCount, getInventoryFieldForCategory, getPlayerCurrentWorldName, getPlayerPositionHeartbeatIntervalMs, getPlayerPresenceBroadcastSignature, getPostgresInventoryFailureMessage, getSocketAddress, getSocketDeviceInfo, getSocketUserAgent, getTrustedMovementModeLabel, getWorldObjectJournalData, handleBulkDropPickup, handleCheckpointActivateUpdate, handleChickenStateUpdate, handleCowStateUpdate, handleDiceRollUpdate, handleDoorMoveUpdate, handleDuckStateUpdate, handleEntranceGateMoveUpdate, handleTackleBoxHarvestUpdate, handleWorldLockMoveUpdate, isPostgresAuthoritativeReady, isValidRespawnTeleportPosition, logDropPickupInventoryIssue, logDropPickupNotAvailable, wasDropJustCollectedByPlayer, acknowledgeDropAlreadyCollected, logDropPickupTooFar, logItemLedgerForState, logWorldChange, makeAuditId, makeRequestId, maybeApplyReciprocalDoorLink, persistAuthoritativeWorldState, persistPlayerInventoryChange, persistWorldStateAfterInventoryCommit, playerNetworkStats, postgresStore, prepareAntiGravityStateUpdate, prepareAntiPunchStateUpdate, prepareAntiTalkStateUpdate, prepareAreaLockStateUpdate, prepareBulletinBoardStateUpdate, prepareDoorStateUpdate, prepareDropPickup, prepareEntrancePassUpdate, prepareMailboxStateUpdate, prepareSpringboardAnimationUpdate, prepareThemeMachineStateUpdate, prepareToggleBlockStateUpdate, prepareWoodenEntranceStateUpdate, prepareWorldLockStateUpdate, queuePlayerPositionBroadcast, queueWorldSave, queueWorldUpdateBroadcast, refreshElectricalVisibilityForWorld, refreshPlayerFishingPresence, rejectIfWorldBanned, releaseLiveActionLock, releasePlayerInventoryLocks, removeUnavailableDropFromWorldState, requireAuthenticated, requireBuildPermission, requireBuildPermissionAtGrid, requireSameWorld, sanitizeDropCreate, sanitizeDropPickup, sanitizeDropUpdate, sanitizeEquipmentSlots, sanitizeInteractionUpdate, sanitizePlayerAnimationState, sanitizePlayerDamageFlash, sanitizePlayerPosition, sanitizePlayerVelocity, sendActionRejected, sendElectricalVisibilityRefresh, sendInventoryTransactionResult, sendWorldUpdateToRequesterAndWorld, serializeWorldState, shouldUseBulkDropPickup, spendItemFromState, touchLivePresence, tradeByPlayerId, usesTrustedMovementPosition, validateDropCreateAgainstServerState, validateDropUpdateAgainstServerState, worldDropActionLocks, worldStates, } = deps;
    const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
    async function acquirePlayerInventoryLocksWithWait(usernames, owner, maxWaitMs = DROP_PICKUP_INVENTORY_LOCK_WAIT_MS, retryMs = DROP_PICKUP_INVENTORY_LOCK_RETRY_MS) {
    ...
    async function handleWorldInteractionUpdate(socket, player, data, context) {
        ...
        if (!requireSameWorld(socket, player, worldName, "edit that world"))
            return;
        if (await rejectIfWorldBanned(socket, player, worldName, "world_interaction_update"))
            return;
        const update = sanitizeInteractionUpdate(data, worldName);
        if (!update)
            return;
        if (update.action === "springboard_animation") {
            if (!prepareSpringboardAnimationUpdate(socket, player, worldName, update))
                return;
        ...
        if (update.action === "entrance_pass") {
            if (!prepareEntrancePassUpdate(socket, player, worldName, update))
                return;
            queueWorldUpdateBroadcast(worldName, update, player.id);
            touchLivePresence(socket, player);
            return;
        ...
        if (update.action === "entrance_gate_move") {
            if (!requireBuildPermission(socket, player, worldName, "move the Entrance Gate"))
                return;
        ...
        if (update.action === "tackle_box_state") {
            if (!requireBuildPermissionAtGrid(socket, player, worldName, update.x, update.y, "world_interaction_update"))
                return;
        ...
        if (update.action === "anti_punch_state") {
            if (!prepareAntiPunchStateUpdate(socket, player, worldName, update))
                return;
        }
        if (update.action === "anti_talk_state") {
            if (!prepareAntiTalkStateUpdate(socket, player, worldName, update))
                return;
        }
        if (update.action === "anti_gravity_state") {
            if (!prepareAntiGravityStateUpdate(socket, player, worldName, update))
                return;
        }
        if (update.action === "wooden_entrance_state") {
            if (!prepareWoodenEntranceStateUpdate(socket, player, worldName, update))
                return;
        }
        else if (update.action === "door_state") {
            if (!prepareDoorStateUpdate(socket, player, worldName, update))
                return;
        }
        else if (update.action === "ceiling_lamp_state") {
            if (!requireBuildPermission(socket, player, worldName, "toggle this lamp"))
                return;
            if (!prepareToggleBlockStateUpdate(socket, player, worldName, update, "ceiling_lamp_state"))
                return;
        }
        else if (update.action === "theme_machine_state") {
            if (!prepareThemeMachineStateUpdate(socket, player, worldName, update))
                return;
        }
        else if (update.action === "world_lock_state") {
            if (!prepareWorldLockStateUpdate(socket, player, worldName, update))
                return;
        }
        else if (update.action === "area_lock_state") {
            if (!prepareAreaLockStateUpdate(socket, player, worldName, update))
                return;
        }
        else if (update.action === "mailbox_state") {
            if (!prepareMailboxStateUpdate(socket, player, worldName, update))
                return;
        }
        else if (update.action === "bulletin_board_state") {
            if (!prepareBulletinBoardStateUpdate(socket, player, worldName, update))
                return;
        }
        ...
        if (update.action === "theme_machine_state" && update.enabled) {
            ...
            for (const interactionUpdate of interactionUpdates) {
                ...
                worldObjectChangeEntries.push(buildWorldObjectChangeEntry(socket, player, worldName, interactionUpdate, objectBefore, objectAfter, interactionSourceId, interactionDetails));
            }
        ...
        const worldObjectChangeEntry = buildWorldObjectChangeEntry(socket, player, worldName, update, objectBefore, objectAfter, interactionSourceId, interactionDetails);
    ...
    async function handleWorldItemDropCreate(socket, player, data, context) {
        if (!requireAuthenticated(socket, player, "edit drops"))
            return;
        const worldName = cleanWorld(data.world || player.world || "START");
        if (!requireSameWorld(socket, player, worldName, "create drops in that world"))
            return;
        if (await rejectIfWorldBanned(socket, player, worldName, "world_item_drop_create"))
            return;
        ...
        if (!update)
            return;
        if (!validateDropCreateAgainstServerState(socket, player, update))
            return;
    ...
    async function handleWorldItemDropUpdate(socket, player, data, context) {
        if (!requireAuthenticated(socket, player, "edit drops"))
            return;
        const worldName = cleanWorld(data.world || player.world || "START");
        if (!requireSameWorld(socket, player, worldName, "edit drops in that world"))
            return;
        if (await rejectIfWorldBanned(socket, player, worldName, "world_item_drop_update"))
            return;
        if (!requireBuildPermission(socket, player, worldName, "edit drops in this locked world"))
            return;
        const update = sanitizeDropUpdate(data, worldName);
        if (!update)
            return;
        if (!validateDropUpdateAgainstServerState(socket, player, worldName, update))
            return;
        const requestId = makeRequestId(data);
        const dropTransactionId = makeAuditId("drop");
        update.server_action_id = dropTransactionId;
    ...
    async function handleWorldItemDropPickup(socket, player, data, context) {
        if (!requireAuthenticated(socket, player, "pick up drops"))
            return;
        const worldName = cleanWorld(data.world || getPlayerCurrentWorldName(player) || player.world || "START");
        if (!requireSameWorld(socket, player, worldName, "pick up drops in that world"))
            return;
        if (await rejectIfWorldBanned(socket, player, worldName, "world_item_drop_pickup"))
            return;
        const requestId = makeRequestId(data);
        ...
        if (isPostgresAuthoritativeReady()) {
            const rejectPickup = (pickupPlan, message, extra = {}) => {
            ...
            const rejectPreparedPickup = (plan) => {
            ...
            try {
                ...
                if (!postgresPickup.ok) {
                    ...
                    if (DropContracts.isPostgresDropPickupCollectedFailure(postgresPickup)) {
                        ...
                        if (removal.ok) {
                            ...
                            await persistAuthoritativeWorldState(pickupPlan.world, serializeWorldState(pickupPlan.world), "drop_pickup_unavailable_cleanup");
                        }
                ...
                const worldPersistResult = await persistAuthoritativeWorldState(pickupPlan.world, postPickupWorldState, "drop_pickup");
    ...
    async function handlePlayerPosition(socket, player, data, context) {
        if (!requireAuthenticated(socket, player, "move online"))
            return;
        ...
        if (!position)
            return;
        if (!requireSameWorld(socket, player, position.world, "move in that world"))
            return;
        ...
        if (!acceptPlayerMovement(socket, player, position, { respawnTeleport, data }))
            return;
