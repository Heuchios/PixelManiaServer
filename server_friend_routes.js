...
function createServerFriendRoutes(deps) {
    ...
    function addAccountName(list, username) {
        ...
        if (clean === "")
            return sanitizeAccountNameArray(list, 200);
        const safe = sanitizeAccountNameArray(list, 200);
        if (!accountNameArrayHas(safe, clean))
            safe.push(clean);
    ...
    function getFriendStatus(viewerUsername, targetUsername) {
        ...
        if (!viewer || target === "")
            return "none";
        if (accountKey(viewer.username) === accountKey(target))
            return "self";
        if (accountNameArrayHas(viewer.friends, target))
            return "friends";
        if (accountNameArrayHas(viewer.friend_requests_out, target))
            return "outgoing";
        if (accountNameArrayHas(viewer.friend_requests_in, target))
            return "incoming";
        return "none";
    }
    ...
    function handleFriendResponse(socket, player, data) {
        if (!requireAuthenticated(socket, player, "answer friend request"))
            return;
