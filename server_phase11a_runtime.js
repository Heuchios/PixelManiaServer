// Generated from src/server_phase11a_runtime.ts. Do not edit by hand.
"use strict";
...
function installConsoleWriteGuard(runtimeProcess = process, runtimeConsole = console) {
...
function createServerPhase11aRuntime(deps) {
    const handleClientRelease = require("./client_releases").createReleaseHandler();
    const { ALLOW_LEGACY_WORLD_STATE_IMPORT, CUSTOM_TRUSTED_PLAYER_STATE_ENABLED, DEV_BACKEND_LOGIN_ALLOWED, DROP_INTEREST_LEAVE_RADIUS_PIXELS, DROP_INTEREST_RADIUS_PIXELS, DROP_INTEREST_SYNC_INTERVAL_MS, dropInterestByReceiver, handleStripeIapWebhook, HOST, IDEMPOTENCY_TTL_MS, IDEMPOTENCY_TTL_MS_COMBAT, IDEMPOTENCY_TTL_MS_CRITICAL, IDEMPOTENCY_TTL_MS_WORLD_ACTION, MAX_MOVE_ACCEL_PIXELS_PER_SECOND2, MAX_MOVE_PIXELS_PER_SECOND, MAX_PACKET_BYTES, MAX_PLAYERS_PER_WORLD, MAX_TRUSTED_POSITION_AGE_MS, MAX_TRUSTED_POSITION_AGE_MS_COMBAT, MAX_TRUSTED_POSITION_AGE_MS_WORLD_ACTION, MIN_CLIENT_VERSION, MIN_PASSWORD_LENGTH, MOVEMENT_COLLISION_GUARD_ENABLED, MOVEMENT_CORRECTION_SNAP_DISTANCE, MOVEMENT_DISTANCE_GRACE_PIXELS, MOVEMENT_MAX_ELAPSED_SECONDS, NETFOX_ARCHIVE_TOOLS_ALLOWED, NETFOX_MOVEMENT_ALLOW_STATIC_FALLBACK, NETFOX_MOVEMENT_ENABLED, NETFOX_MOVEMENT_MAX_CLIENTS, NETFOX_MOVEMENT_PUBLIC_HOST, NETFOX_MOVEMENT_PUBLIC_PORT, NETFOX_MOVEMENT_ROUTE_TTL_MS, NETFOX_SPAWN_TICKET_TTL_MS, NETFOX_TRUSTED_PLAYER_STATE_ENABLED, NEWS_DATA_PATH, PACKET_SIZE_TELEMETRY_ENABLED, PACKET_TYPE_SIZE_SAMPLE_LIMIT, PLAYER_ACTION_INTEREST_MANAGEMENT_ENABLED, PLAYER_INTEREST_LEAVE_RADIUS_PIXELS, PLAYER_INTEREST_RADIUS_PIXELS, PLAYER_POSITION_BATCHING_ENABLED, PLAYER_POSITION_BATCH_MAX_ITEMS, PLAYER_POSITION_BATCH_MIN_CLIENT_VERSION, PLAYER_POSITION_BROADCAST_INTERVAL_MS, PLAYER_POSITION_DELIVERY_RETRY_MS, PLAYER_POSITION_IDLE_HEARTBEAT_MS, PLAYER_POSITION_MAX_BUFFERED_AMOUNT, PLAYER_POSITION_RESUME_BUFFERED_AMOUNT, PORT, POSTGRES_AUTHORITATIVE, POSTGRES_ENABLED, POSTGRES_SCHEMA, PUBLIC_BASE_URL, PUBLIC_WS_URL, REDIS_ENABLED, REDIS_REQUIRED_FOR_ROUTE_ENFORCEMENT, REQUIRE_POSTGRES_AUTHORITATIVE_FOR_GAMEPLAY, SAVE_DEBOUNCE_MS, SERVER_CLIENT_VERSION, SERVER_INSTANCE_ID, SERVER_INSTANCE_WS_URL, SERVER_TICK_MONITOR_INTERVAL_MS, SMTP_HOST, TRUSTED_MOVEMENT_ALLOWLIST, TRUSTED_MOVEMENT_ALLOWLIST_ENABLED, TRUSTED_MOVEMENT_REQUIRE_JOINED_WORLD, WORLD_ADMISSION_TTL_MS, WORLD_JSON_BACKUP_DEBOUNCE_MS, WORLD_JSON_BACKUP_WHEN_PG_READY, WORLD_NON_CRITICAL_WORLD_SAVE_DEBOUNCE_MS, WORLD_ROUTE_ENFORCEMENT_ENABLED, WORLD_ROUTE_TTL_MS, WORLD_SNAPSHOT_INTERVAL_MINUTES, WORLD_SNAPSHOT_INTERVAL_MS, WORLD_SNAPSHOT_MAX_WORLDS_PER_CYCLE, WORLD_SNAPSHOT_POSTGRES_INLINE, WORLD_SNAPSHOT_SPACES_ENDPOINT, WORLD_SNAPSHOT_SPACES_TARGET, WORLD_SNAPSHOT_STARTUP_RUN, WORLD_SNAPSHOT_STORAGE, WORLD_UPDATE_BATCHING_ENABLED, WORLD_UPDATE_BATCH_INTERVAL_MS, WORLD_UPDATE_BATCH_MAX_ITEMS, WORLD_UPDATE_BATCH_MIN_CLIENT_VERSION, applyPasswordResetToken, assertAuthoritativePostgresReady, buildNetfoxWorldStateHttpPayload, cleanWorld, confirmEmailChangeToken, cryptoRandomUUID, CRASH_REPORT_PATH, errorToCrashDetails, escapeHtml, exitProcess, fileSystem, getNetfoxMovementRouteForWorld, getNetfoxMovementRouteStats, getWorldIndexStatsSnapshot, getWorldRouteStatsSnapshot, getWorldSnapshotSchedulerRunning, httpServer, isCustomMovementServerWorldStateEndpointConfigured, isDropInterestManagementEnabled, isNetfoxServerWorldStateEndpointConfigured, isNetfoxSpawnTicketConfigured, isPlayerInterestManagementEnabled, loadPersistentState, logger, markFatalCrashReportWritten, pathModule, pendingPersistenceWrites, pendingPlayerPositionBroadcasts, pendingWorldJsonBackups, pendingWorldUpdateBroadcasts, playerInterestByReceiver, playerNetworkStats, playerStates, players, postgresStore, processRuntime, recoverWorldEventsAfterLoad, redisStore, refreshWorldDropsFromPostgres, registerNetfoxMovementRoute, serverRuntimeStats, serverTickStats, startAntiDupeAuditScanner, startCalendarEventScheduler, startPeriodicWorldSnapshotScheduler, startWorldEventRandomScheduler, verifyCustomMovementServerWorldStateRequest, verifyEmailToken, verifyNetfoxServerWorldStateRequest, verifyNetfoxSpawnTicketPayload, worldJsonBackupTimers, worldNetworkStats, worldSaveTimers, worldSnapshotSchedulerState, worldSnapshotStorageIsSpaces, worldStates, wss, } = deps;
    ...
    function recordPacketTypeSize(direction, rawMessageType, rawBytes) {
    ...
    function getPlayerNetworkStatsSnapshot() {
        const inboundPacketTypeStats = serverRuntimeStats.getPacketTypeSizeStatsSnapshot(playerNetworkStats.inbound_packet_type_stats);
        const outboundPacketTypeStats = serverRuntimeStats.getPacketTypeSizeStatsSnapshot(playerNetworkStats.outbound_packet_type_stats);
        return {
            ...
            inbound_message_queue_max_socket_depth: Number(playerNetworkStats.inbound_message_queue_max_socket_depth || 0),
            ...
            inbound_message_queue_wait_avg_ms: Number(playerNetworkStats.inbound_message_queue_wait_samples || 0) > 0
                ? Number((Number(playerNetworkStats.inbound_message_queue_wait_total_ms || 0)
                    / Number(playerNetworkStats.inbound_message_queue_wait_samples || 1)).toFixed(3))
                : 0,
            inbound_message_queue_wait_max_ms: Number(playerNetworkStats.inbound_message_queue_wait_max_ms || 0),
            coalesced_inbound_player_position_messages: Number(playerNetworkStats.coalesced_inbound_player_position_messages || 0),
            ...
            player_position_queue_wait_avg_ms: Number(playerNetworkStats.player_position_queue_wait_samples || 0) > 0
                ? Number((Number(playerNetworkStats.player_position_queue_wait_total_ms || 0)
                    / Number(playerNetworkStats.player_position_queue_wait_samples || 1)).toFixed(3))
                : 0,
            player_position_queue_wait_max_ms: Number(playerNetworkStats.player_position_queue_wait_max_ms || 0),
            player_position_queue_wait_over_250ms: Number(playerNetworkStats.player_position_queue_wait_over_250ms || 0),
            ...
            movement_backpressure_replaced_items: Number(playerNetworkStats.movement_backpressure_replaced_items || 0),
            movement_backpressure_flushes: Number(playerNetworkStats.movement_backpressure_flushes || 0),
            movement_backpressure_dropped_items: Number(playerNetworkStats.movement_backpressure_dropped_items || 0),
    ...
    function startServerTickMonitor() {
        ...
        serverTickMonitorTimer = setInterval(() => {
            const now = processRuntime.hrtime.bigint();
            ...
            serverRuntimeStats.applyServerTickSample(serverTickStats, elapsedMs, SERVER_TICK_MONITOR_INTERVAL_MS);
        }, SERVER_TICK_MONITOR_INTERVAL_MS);
        if (typeof serverTickMonitorTimer.unref === "function")
            serverTickMonitorTimer.unref();
    }
...
<body><main><h1>${escapeHtml(title)}</h1><p>${escapeHtml(message)}</p></main></body>
    ...
    function sendPasswordResetForm(response, token, message = "") {
...
<body>
  <main>
    ...
    function readFormHttpRequestBody(request, maxBytes = 16384) {
    ...
    function readJsonHttpRequestBody(request, maxBytes = 16384) {
        return new Promise((resolve, reject) => {
            const chunks = [];
            let totalBytes = 0;
            request.on("data", (chunk) => {
    ...
    async function handleNetfoxVerifySpawnTicketHttpRequest(request, response) {
    ...
    async function handleNetfoxRegisterRouteHttpRequest(request, response) {
        if (!isNetfoxServerWorldStateEndpointConfigured()) {
            sendHttpJson(response, 503, {
                ok: false,
                error: "Netfox server authentication is not configured.",
    ...
    async function handleNetfoxGetRouteHttpRequest(request, response, url) {
        if (!isNetfoxServerWorldStateEndpointConfigured()) {
            sendHttpJson(response, 503, {
                ok: false,
                error: "Netfox server authentication is not configured.",
    ...
    async function handleHttpRequest(request, response) {
        ...
        if (handleClientRelease(request, response, url))
            return;
        ...
        if (request.method === "POST" && url.pathname === "/reset-password") {
            ...
            try {
                ...
                sendHtml(response, getErrorMessage(error) === "request_body_too_large" ? 413 : 400, "Reset Failed", "Could not read this password reset form.");
                return;
            }
            ...
            sendHtml(response, result.ok ? 200 : 400, result.ok ? "Password Changed" : "Reset Failed", result.message);
            return;
        }
        if (request.method === "GET" && url.pathname === "/change-email") {
            const result = await confirmEmailChangeToken(url.searchParams.get("token") || "");
            sendHtml(response, result.ok ? 200 : 400, result.ok ? "Email Changed" : "Email Change Failed", result.message);
            return;
        }
