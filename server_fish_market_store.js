      ...
      created_at timestamptz NOT NULL DEFAULT now(), PRIMARY KEY (player_id, request_id));`);
        ...
        if (!speciesMigration.rowCount) {
            ...
            DO UPDATE SET amount=$4, stack_limit=20000, row_version=${store.table("inventory")}.row_version+1, updated_at=now()`, [row.player_id, target, amount, before + amount]);
              ...
              VALUES ($1,'system','fish_species_migration',$2,'fish',$3,$4,$5,$6::jsonb)`, [row.player_id, id, delta, oldAmount, newAmount, JSON.stringify({ from_item: row.item_type, to_item: target, inventory_unit: Market.UNIT })]);
                    }
            ...
            for (const row of worlds.rows) {
                const before = JSON.parse(JSON.stringify(row.world_state));
                if (!Market.migrateWorldSpecies(row.world_state))
                    continue;
                const checksumState = { ...row.world_state };
                delete checksumState.saved_at;
                delete checksumState.last_saved_at;
            ...
            for (const target of new Set(Object.values(Market.SPECIES_ALIASES))) {
                const related = marketRows.rows.filter((row) => (Market.SPECIES_ALIASES[row.item_id] || row.item_id) === target);
                if (!related.some((row) => Market.SPECIES_ALIASES[row.item_id]))
                    continue;
                const supply = related.reduce((sum, row) => sum + Market.quote(row, Market.policy(ItemDatabase.getItemDefinition(row.item_id)), now).supply, 0);
...
async function lockSale(store, client, metadata, deltas, playerId = "", requestId = "") {
    ...
    if (!gem || deltas.length !== lines.length + 1)
    ...
    for (const line of [...lines].sort((a, b) => a.item_id.localeCompare(b.item_id))) {
        ...
        if (!row)
            throw new Error("Fish market unavailable.");
        ...
        if (!Number.isSafeInteger(line.amount) || line.amount <= 0 || line.amount > Market.STACK_LIMIT ||
            !deltas.some(d => d.item_type === line.item_id && d.item_category === "fish" && d.delta === -line.amount))
            throw new Error("Invalid fish weight");
    ...
    for (const line of settled) {
        ...
        await client.query(`UPDATE ${store.table("fish_market")} SET supply = $2, updated_ms = $3, revision = revision + 1 WHERE item_id = $1`, [line.item_id, line.supply + line.amount, line.quoted_ms]);
    }
}
