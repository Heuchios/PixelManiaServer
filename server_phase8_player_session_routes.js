...
const WORLD_ENTRY_MAX_SNAPSHOT_RESTARTS = Math.max(2, Math.trunc(Number(process.env.WORLD_ENTRY_MAX_SNAPSHOT_RESTARTS) || 6));
...
function createServerPhase8PlayerSessionRoutes(deps) {
    ...
    async function activateWorldEntry(socket, player, context) {
        ...
        if (!worldName || !sessionId || player.world_entry_state === "active" || player.world_entry_ticket_pending)
            return;
    ...
    async function handleJoinWorld(socket, player, data, context) {
        ...
        if (/^LANDFILL_/i.test(newWorld)) {
            if (!await deps.authorizeLandfillTicket(socket, player, newWorld, false))
                return;
