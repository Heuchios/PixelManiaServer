...
Write-Host "== PixelManiaServer: build + commit appreciation_wings atlas migration (server) ==" -ForegroundColor Cyan
...
$commitMessage = @"
Make appreciation_wings fully atlas-driven (server mirror)

- Mirror the client's item_database.gd change: appreciation_wings' texture/inventory_icon
  and idle_frames/jump_frames/fall_frames/flap_frames now reference the wearable atlas frame
  keys (appreciation_wings_icon, appreciation_wings_1..5) instead of
  res://Assets/items/back_items/appreciation_wings_*.png. sprite_folder/idle_sprite dropped,
  matching the client entry. Frame ordering unchanged.
- scripts/check_item_database_sync.js only checks item-id parity so this was not required for
  check:item-db to pass, but keeps the two databases from drifting for admin lookup tooling.
"@
