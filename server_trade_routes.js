...
function createServerTradeRoutes(deps) {
    ...
    async function handleTradeRequest(socket, player, data) {
        if (!requireAuthenticated(socket, player, "trade"))
            return;
    ...
    function findTradeForResponse(player, data) {
        ...
        if (explicitTradeId !== "")
            return activeTrades.get(explicitTradeId) || null;
        ...
        if (requesterUsername !== "") {
            for (const trade of activeTrades.values()) {
                if (trade.status !== "pending")
                    continue;
                if (trade.target_id !== player.id)
                    continue;
                if (accountKey(trade.requester_username) !== accountKey(requesterUsername))
                    continue;
                return trade;
            }
        }
    ...
    function getTradeOfferTotals(slots, overrideSlot = -1, overrideItem = null) {
        ...
        for (let i = 0; i < TRADE_SLOT_COUNT; i += 1) {
            ...
            if (itemId === "" || itemCategory === "" || amount <= 0)
                continue;
            if (!ItemDatabase.canStoreItemInCategory(itemId, itemCategory))
                continue;
    ...
    function canOfferTradeItems(username, slots, overrideSlot = -1, overrideItem = null) {
        ...
        if (!state)
            return { ok: false, message: "Could not load server inventory." };
    ...
    async function callTradeRoute(route, socket, player, data) {
