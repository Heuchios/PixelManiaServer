// Generated from src/postgres_store_contracts.ts. Do not edit by hand.
...
function stableNormalizeForHash(value) {
    ...
    if (value instanceof Date)
        return normalizeOptionalTimestamp(value) || value.toISOString();
    if (Array.isArray(value))
        return value.map((item) => stableNormalizeForHash(item));
    if (typeof value === "object") {
        const source = value;
        const normalized = {};
...
function normalizeTransactionLedgerStatus(value, fallback = "success") {
    ...
    if (TRANSACTION_LEDGER_STATUSES.has(clean))
        return clean;
...
function shouldTrackItemInstance(itemType, itemCategory = "") {
    ...
    const definition = typeof ItemDatabase.getItemDefinition === "function"
        ...
        : null;
    if (definition && definition.instance_tracked === false)
        return false;
    if (definition && (definition.instance_tracked === true || definition.equipable || cleanName(definition.equipment_slot) !== ""))
        return true;
    if (ITEM_INSTANCE_TRACKED_CATEGORIES.has(resolvedCategory))
        return true;
...
function normalizeItemInstanceState(value, fallback = ITEM_INSTANCE_ACTIVE_STATE) {
    const clean = cleanName(value).toLowerCase();
    if (ITEM_INSTANCE_STATES.has(clean))
        return clean;
...
function normalizeItemInstanceLocation(value, fallback = "inventory") {
    const clean = cleanName(value).toLowerCase();
    if (ITEM_INSTANCE_LOCATIONS.has(clean))
        return clean;
...
function normalizeTransactionLedgerType(entry = {}) {
    ...
    if (source === "vending") {
        if (action.includes("buy") || action.includes("spend") || action.includes("receive") || action.includes("payment"))
            return "VENDING_BUY";
        if (action.includes("list"))
            return "VENDING_LIST";
        if (action.includes("cancel"))
            return "VENDING_CANCEL";
        if (action.includes("collect"))
            return "VENDING_COLLECT";
        return "VENDING_TRANSACTION";
    }
    if (source === "admin") {
        if (action.includes("remove") || delta < 0)
            return "ADMIN_REMOVE_ITEM";
        if (action.includes("give") || delta > 0)
            return "ADMIN_GIVE_ITEM";
        return "ADMIN_ITEM_ACTION";
    }
    if (source === "craft" || source === "crafting")
        return delta >= 0 ? "CRAFT_OUTPUT" : "CRAFT_INPUT";
    if (source === "furnace")
        return delta >= 0 ? "FURNACE_OUTPUT" : "FURNACE_INPUT";
    if (source === "fishing" || source === "fish_monger")
        return delta >= 0 ? "FISHING_REWARD" : "FISHING_COST";
    if (source === "drop_inventory")
        return "ITEM_DROP";
    if (source === "drop_pickup" || source === "world_drop")
        return "ITEM_PICKUP";
    if (source === "world_block_place") {
        if (itemType === "world_lock" || itemType.endsWith("_lock") || itemCategory === "lock")
            return "WORLD_LOCK_PLACE";
        return "WORLD_BLOCK_PLACE";
    }
    if (source === "world_lock_key")
        return delta >= 0 ? "WORLD_LOCK_KEY_ISSUE" : "WORLD_LOCK_KEY_CONSUME";
    if (source === "world_block_break")
        return "WORLD_BLOCK_BREAK";
    if (source === "safe") {
        if (action.includes("deposit") || delta < 0)
            return "SAFE_DEPOSIT";
        if (action.includes("withdraw") || delta > 0)
            return "SAFE_WITHDRAW";
        return "SAFE_TRANSACTION";
    }
    if (source === "display") {
        if (action.includes("deposit") || delta < 0)
            return "DISPLAY_DEPOSIT";
        if (action.includes("withdraw") || delta > 0)
            return "DISPLAY_WITHDRAW";
...
function normalizeItemInstanceEventType(value, fallback = "updated") {
    const clean = cleanName(value).toLowerCase();
    if (ITEM_INSTANCE_EVENT_TYPES.has(clean))
        return clean;
...
function normalizeWorldDropPayload(drop = {}, fallback = {}) {
    ...
    if (dropId === "" || itemType === "" || !ItemDatabase.hasItem(itemType))
        return null;
...
function worldLockRowToPayload(row = {}) {
    ...
    if (!isLocked || cleanLockType === "" || cleanLockType === "none")
        return {};
    ...
    const allowedPlayers = Array.isArray(metadata.allowed_players) ? metadata.allowed_players : [];
    const allowedAccountIds = Array.isArray(metadata.allowed_account_ids) ? metadata.allowed_account_ids : [];
    const allowedPlayerIds = Array.isArray(metadata.allowed_player_ids) ? metadata.allowed_player_ids : [];
    const playerRoles = toObject(metadata.player_roles);
    const playerRolesByAccountId = toObject(metadata.player_roles_by_account_id);
    const playerRolesByPlayerId = toObject(metadata.player_roles_by_player_id);
    return {
        is_locked: true,
...
function normalizeWorldObjectType(entry = {}) {
    ...
    if (explicit !== "" && explicit !== "world_interaction_update")
        return explicit;
    const action = normalizeWorldObjectAction(e.action || "");
    if (action.includes("vend"))
        return "vending";
    if (action.includes("safe"))
        return "safe";
    if (action.includes("mailbox"))
        return "mailbox";
    if (action.includes("bulletin_board"))
        return "bulletin_board";
    if (action.includes("display"))
        return "display";
    if (action.includes("water_well"))
        return "water_well";
    if (action.includes("tackle_box"))
        return "tackle_box";
    if (action.includes("duck"))
        return "duck";
    if (action.includes("dice"))
        return "dice";
    if (action.includes("anti_punch"))
        return "anti_punch";
    if (action.includes("anti_talk"))
        return "anti_talk";
    if (action.includes("anti_gravity"))
        return "anti_gravity";
    if (action.includes("theme_machine"))
        return "theme_machine";
    if (action.includes("cctv"))
        return "cctv";
    if (action === "world_lock_state" || action.includes("world_lock"))
        return "world_lock";
    if (action === "sign_text" || action.includes("sign"))
        return "sign";
    if (action === "door_state" || action.includes("door"))
        return "door";
    if (action === "wooden_entrance_state" || action.includes("entrance"))
        return "wooden_entrance";
    if (action.includes("lamp") || action.includes("toggle"))
        return "toggle";
    return "interaction";
}
function normalizeWorldObjectId(entry = {}, worldName = "", objectType = "") {
    ...
    if (doorId !== "")
        return `door:${doorId}`.slice(0, 160);
    ...
    if (x !== null && y !== null)
        return `${cleanObjectType}:${x}:${y}`.slice(0, 160);
    return `${cleanObjectType}:unknown`.slice(0, 160);
}
function shouldTreatAsWorldObjectChange(entry = {}) {
    const e = toObject(entry);
    if (cleanName(e.object_type || e.object_id || "") !== "")
        return true;
    if (Object.keys(toObject(e.old_data)).length > 0 || Object.keys(toObject(e.new_data)).length > 0)
        return true;
    const action = normalizeWorldObjectAction(e.action || "");
...
function extractWorldObjectJournalMap(worldState = {}, fallbackWorldName = "") {
    ...
    for (const rawInteraction of interactions) {
        ...
        if (!WORLD_OBJECT_CHANGE_ACTIONS.has(action))
            continue;
...
function normalizeLedgerSource(value) {
    ...
    if (raw.includes("quest"))
        return "quest";
    if (raw.includes("crafting") || raw.includes("craft"))
        return "crafting";
    if (raw.includes("loot_box") || raw.includes("lootbox"))
        return "loot_box";
    if (raw.includes("reward"))
        return "reward";
    if (raw.includes("seed_place"))
        return "seed_place";
    if (raw.includes("seed_splice"))
        return "seed_splice";
    if (raw.includes("seed_harvest"))
        return "seed_harvest";
    if (raw.includes("drop_pickup"))
        return "drop_pickup";
    if (raw.includes("drop_create") || raw.includes("drop_from_inventory") || raw.includes("drop_inventory"))
        return "drop_inventory";
    if (raw.includes("furnace"))
        return "furnace";
    if (raw.includes("fishing"))
        return "fishing";
    if (raw.includes("fish_monger"))
        return "fish_monger";
    if (raw.includes("admin") || raw.includes("developer"))
        return "admin";
    if (raw.includes("rollback"))
        return "rollback";
    if (raw.includes("world_block_break"))
        return "world_block_break";
    if (raw.includes("world_block_place"))
        return "world_block_place";
    if (raw.includes("world_lock_conversion") || raw.includes("convert_world_lock"))
        return "world_lock_conversion";
    if (raw.includes("world_lock_key") || raw.includes("world_key"))
        return "world_lock_key";
    if (raw.includes("entrance"))
        return "world_interaction";
    if (raw.includes("world_interaction"))
        return "world_interaction";
    return "system";
}
function normalizeSecuritySeverity(value) {
    ...
    if (raw === "low" || raw === "medium" || raw === "high" || raw === "critical")
        return raw;
    if (raw === "info" || raw === "notice" || raw === "debug")
        return "low";
...
module.exports = PostgresContracts;
