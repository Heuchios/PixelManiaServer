...
Write-Host "== PixelManiaServer: build + commit void_saber atlas migration (server) ==" -ForegroundColor Cyan
...
$commitMessage = @"
Make void_saber fully atlas-driven (server mirror)

- Mirror the client's item_database.gd change: void_saber's texture/inventory_icon now
  reference the wearable atlas frame keys (void_saber_icon, void_saber_1) instead of
  res://Assets/items/swords/void_saber*.png files. Server does not mirror
  hand_item_animations (matching the existing pattern for every other hand item --
  the server database only needs the static texture/icon references).
"@
