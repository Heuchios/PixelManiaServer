...
function getWorldStateStreamLimits(options) {
    const maxPacketBytes = Math.max(4096, Math.trunc(Number(options.maxPacketBytes) || 64 * 1024));
    const targetPacketBytes = Math.min(maxPacketBytes, Math.max(4096, Math.trunc(Number(options.targetPacketBytes) || 48 * 1024)));
...
function prepareWorldStateStreamSection(name, kind, value, targetPacketBytes, maxPacketBytes) {
...
function prepareWorldStateStreamSections(rawState, options = {}) {
...
function buildWorldStateStreamPackets(rawState, options = {}) {
    if (!isRecord(rawState)) {
    ...
    const chunkPayloads = [];
    const makeChunkEnvelope = (sectionName, sectionKind, data) => ({
    ...
    for (const [name, value] of Object.entries(state)) {
        ...
        if (Array.isArray(value) || isRecord(value)) {
            const kind = Array.isArray(value) ? "array" : "dictionary";
    ...
    const chunks = chunkPayloads.map((chunk, index) => ({
        ...chunk.packet,
        chunk_index: index,
...
function createWorldStateHelpers(config) {
    ...
    const currentWorldGenerationVersion = Math.max(legacyWorldGenerationVersion, Math.trunc(Number(config.currentWorldGenerationVersion) || legacyWorldGenerationVersion));
    ...
    function normalizeGrid(rawEntry) {
        ...
        if (!config.isGridInWorld(gridX, gridY))
            return null;
    ...
    function sanitizeWorldBackgroundTheme(value) {
    ...
    function normalizeBlockEntry(rawEntry) {
        ...
        if (blockType.length === 0)
            return null;
        if (blockType === "crafting_station_right")
            return null;
        if (blockType === "crafting_station_left")
            blockType = "crafting_station";
        if (!itemDatabase.hasItem(blockType) || resolveInventoryCategory(blockType) !== "block")
            return null;
        const entry = {
            x: grid.x,
            y: grid.y,
    ...
    function cleanElectricalSignalMode(value) {
        const clean = clampString(value || "").toLowerCase();
        return config.electricalSignalModes.has(clean) ? clean : "on_off";
    }
    function getElectricalDeviceTypeForItem(itemId, rawDeviceType = "") {
        const cleanItem = clampString(itemId || "").toLowerCase();
        const cleanDeviceType = clampString(rawDeviceType || "").toLowerCase();
        if (config.electricalValidDeviceTypes.has(cleanDeviceType))
            return cleanDeviceType;
        if (cleanItem === config.electricalWireItem)
            return config.electricalDeviceWire;
        if (cleanItem === config.electricalMetalPadItem)
            return config.electricalDeviceMetalPad;
        if (cleanItem === config.electricalGeneratorItem)
            return config.electricalDeviceGenerator;
        if (cleanItem === config.electricalPoleItem)
            return config.electricalDevicePole;
        return "";
    }
    function normalizeElectricalLinkedGridKeys(rawValue, maxCount) {
        ...
        for (const rawEntry of source) {
            ...
            if (!grid || !Number.isFinite(grid.x) || !Number.isFinite(grid.y))
                continue;
            if (!config.isGridInWorld(grid.x, grid.y))
                continue;
            ...
            seen.add(key);
            linkedKeys.push(key);
            if (linkedKeys.length >= maxCount)
                break;
        }
        return linkedKeys;
    }
    function normalizeElectricalLinkedPadKeys(rawValue) {
    ...
    function normalizeElectricalLinkedPoleKeys(rawValue) {
        const source = Array.isArray(rawValue)
            ? rawValue
    ...
    function normalizeElectricPoleLinkedPoleKeys(rawValue, selfKey = "") {
        const source = Array.isArray(rawValue)
            ? rawValue
            : (Array.isArray(rawValue?.linked_pole_keys) ? rawValue.linked_pole_keys : []);
    ...
    function isElectricalLayerItem(itemId) {
        const cleanItem = clampString(itemId || "").toLowerCase();
        if (!config.electricalItemTypes.has(cleanItem))
            return false;
    ...
    function isElectricalDeviceBlockItem(itemId) {
        const cleanItem = clampString(itemId || "").toLowerCase();
        if (cleanItem !== config.electricalMetalPadItem &&
    ...
    function getElectricalDeviceBlockLayer(itemId) {
        const cleanItem = clampString(itemId || "").toLowerCase();
        if (cleanItem === config.electricalMetalPadItem)
            return "background";
        if (cleanItem === config.electricalGeneratorItem)
    ...
    function normalizeElectricalEntry(rawEntry) {
        if (!isRecord(rawEntry))
            return null;
        const grid = normalizeGrid(rawEntry);
        if (!grid)
    ...
    function normalizeElectricalDeviceStateEntry(rawEntry) {
        if (!isRecord(rawEntry))
            return null;
        const grid = normalizeGrid(rawEntry);
        if (!grid)
            return null;
        const itemId = clampString(rawEntry.item_id || rawEntry.item_type || rawEntry.block_type || rawEntry.type || "").toLowerCase();
        if (!isElectricalDeviceBlockItem(itemId))
            return null;
        const deviceType = getElectricalDeviceTypeForItem(itemId, rawEntry.device_type || rawEntry.electrical_device_type || "");
        if (deviceType !== config.electricalDeviceMetalPad &&
    ...
    function putElectricalDeviceEntryInState(state, deviceEntry) {
    ...
    function loadElectricalDataIntoState(state, data, worldName = "") {
        void worldName;
        if (!isRecord(state) || !isRecord(data))
            return;
        ...
        for (const rawEntry of rawLayerEntries) {
            ...
            if (isElectricalLayerItem(itemId)) {
                ...
                if (entry)
                    electrical.set(gridKey(entry.x, entry.y), entry);
                continue;
            }
            if (!isElectricalDeviceBlockItem(itemId))
                continue;
            ...
            if (deviceEntry)
                putElectricalDeviceEntryInState(state, deviceEntry);
        }
    ...
    function sanitizeElectricalLayerUpdate(data, worldName) {
        const action = clampString(data.action || "").toLowerCase();
        if (action !== "place" && action !== "break" && action !== "remove")
            return null;
        ...
        if (action === "place" && !isElectricalLayerItem(blockType))
            return null;
        if (blockType !== "" && !isElectricalLayerItem(blockType))
            return null;
        return {
    ...
    function serializeSeedForSave(seed) {
        ...
        if (!isRecord(seed) || !isRecord(serialized))
            return serialized;
    ...
    function loadGridArrayIntoMap(target, rawEntries, normalizeEntry) {
        if (!(target instanceof Map) || !Array.isArray(rawEntries) || typeof normalizeEntry !== "function")
            return;
    ...
    function getBedrockStartY() {
        const rawStart = Math.trunc(Number(config.bedrockStartY));
        if (!Number.isFinite(rawStart))
            return Math.max(0, Math.trunc(Number(config.worldHeight) || 0));
    ...
    function applyLoadedWorldClearState(state) {
    ...
    function loadSavedWorldGridData(state, data) {
        if (!isRecord(state) || !isRecord(data))
            return;
    ...
    function loadWorldEventStateIntoState(state, data) {
        ...
        if (eventType !== config.snowStormEventType)
            return;
    ...
    function sanitizeVendListing(rawListing) {
        ...
        if (stock <= 0 || amountPerSale <= 0 || stock < amountPerSale)
            return null;
        return {
            item_id: itemId,
    ...
    function sanitizeVendLogEntry(rawEntry) {
        ...
        if (buyerName === "" || itemId === "" || !itemDatabase.hasItem(itemId))
            return null;
    ...
    function canStoreItemInSafe(itemId, itemCategory) {
        ...
        if (cleanItemId === "" || !itemDatabase.hasItem(cleanItemId))
            return false;
        if (cleanItemId === "punch" || cleanItemId === config.safeBlockType || cleanItemId === config.worldLockKeyItemType)
            return false;
    ...
    function sanitizeSafeSlot(rawSlot) {
        ...
        if (!canStoreItemInSafe(itemId, rawSlot.item_category || rawSlot.category || ""))
            return null;
    ...
    function makeEmptyDisplayState(worldName, x, y) {
        return {
    ...
    function sanitizeDisplaySlot(rawSlot) {
        if (!isRecord(rawSlot))
            return null;
        const itemId = clampString(rawSlot.item_id || rawSlot.item_type || rawSlot.item || "");
        if (!canStoreItemInDisplay(itemId, rawSlot.item_category || rawSlot.category || ""))
            return null;
        const itemCategory = resolveInventoryCategory(itemId, rawSlot.item_category || rawSlot.category || "");
        const slot = {
            item_id: itemId,
    ...
    function makeEmptyMailboxState(worldName, x, y) {
        return {
    ...
    function sanitizeMailboxMessage(rawMessage) {
    ...
    function sanitizeMailboxState(rawEntry, worldName, x, y) {
    ...
    function makeEmptyBulletinBoardState(worldName, x, y) {
        return {
    ...
    function sanitizeBulletinBoardMessage(rawMessage) {
        if (!isRecord(rawMessage))
            return null;
        const message = String(rawMessage.message || rawMessage.text || "").trim().slice(0, config.maxBulletinBoardMessageLength);
        if (message === "")
            return null;
        const playerName = cleanAccountName(rawMessage.player_name || rawMessage.username || rawMessage.from || rawMessage.sender || "Player").toUpperCase() || "PLAYER";
        return {
    ...
    function sanitizeBulletinBoardState(rawEntry, worldName, x, y) {
    ...
    function parseObjectTimestampMs(value) {
    ...
    function makeEmptyTackleBoxState(worldName, x, y, cooldownMs = config.tackleBoxCooldownMs) {
        return {
    ...
    function sanitizeTackleBoxState(rawEntry, worldName, x, y, cooldownMs = config.tackleBoxCooldownMs) {
    ...
    function makeEmptyAnimalState(action, worldName, x, y, productionMs, hungerMs) {
        return {
            action,
            world: cleanWorld(worldName),
            x: Math.trunc(Number(x) || 0),
    ...
    function normalizeAnimalStatus(rawStatus, nextHarvestAtMs, diesAtMs, nowMs = Date.now()) {
        const cleanStatus = clampString(rawStatus || "").toLowerCase();
        if (nextHarvestAtMs > 0) {
            return nowMs >= nextHarvestAtMs ? "ready" : "producing";
        }
        if (cleanStatus === "ready" || cleanStatus === "producing")
            return cleanStatus;
        if (diesAtMs > 0 || cleanStatus === "hungry")
            return "hungry";
        return "hungry";
    }
    function sanitizeAnimalState(rawEntry, worldName, x, y, action, productionDefault, hungerDefault) {
    ...
    function sanitizeDiceState(rawEntry, worldName, x, y) {
    ...
    function sanitizeToggleObjectState(rawEntry, worldName, x, y, action, fallbackBlockType) {
    ...
    function getLinkedPoleKeyFromTimedMachineState(rawState) {
        ...
        if (!config.isGridInWorld(poleX, poleY))
            return "";
        return gridKey(poleX, poleY);
    }
    function getOilRefineryBatteryCountFromWatts(watts) {
    ...
    function getOilRefineryBatteryWattsFromRaw(rawState) {
        const rawWatts = Number(rawState.battery_watts ??
        ...
        const rawCount = Number(rawState.battery_count ??
    ...
    function sanitizeOilRefineryState(rawEntry = {}, worldName = "", fallbackX = 0, fallbackY = 0) {
    ...
    function sanitizeBatteryChargerState(rawEntry = {}, worldName = "", fallbackX = 0, fallbackY = 0) {
        const rawState = isRecord(rawEntry) ? rawEntry : {};
        const x = clampInteger(rawState.x ?? fallbackX, 0, config.worldWidth - 1);
        const y = clampInteger(rawState.y ?? fallbackY, 0, config.worldHeight - 1);
    ...
    function sanitizeBlockUpdate(data, worldName) {
        ...
        if (action !== "place" && action !== "break" && action !== "hit")
            return null;
        ...
        if (atlasItemId > 0) {
            ...
            if (atlasItemKey !== "") {
                if (blockType !== "" && blockType !== atlasItemKey)
                    return null;
                blockType = atlasItemKey;
                resolvedAtlasItemId = atlasItemId;
            }
        }
        if (action === "place" && blockType.length === 0)
            return null;
        if (blockType !== "" && (!itemDatabase.hasItem(blockType) || resolveInventoryCategory(blockType) !== "block"))
            return null;
    ...
    function sanitizeInteractionGridAction(data, worldName, action) {
        const grid = normalizeGrid(data);
        if (!grid)
            return null;
        return {
    ...
    function sanitizeInteractionUpdate(data, worldName) {
        ...
        if (action === "entrance_gate_move" || action === "world_lock_move" || action === "door_move") {
            ...
            if (![x, y, oldX, oldY].every(Number.isFinite))
                return null;
            const gridX = Math.trunc(x);
            const gridY = Math.trunc(y);
            const oldGridX = Math.trunc(oldX);
            const oldGridY = Math.trunc(oldY);
            if (!config.isGridInWorld(gridX, gridY) || !config.isGridInWorld(oldGridX, oldGridY))
                return null;
            const sanitized = {
                type: "world_interaction_update",
                world: cleanWorld(worldName),
    ...
    function getStateMap(state, field) {
    ...
    function getEmbeddedInteractionState(update) {
    ...
    function applyInteractionUpdateToWorldState(worldName, update) {
        ...
        if (update.action === "door_state") {
            ...
            const nextPassword = passwordDoor
                ...
                : "";
            const hasDoorLink = config.cleanDoorId(update.door_id || "") !== "" ||
    ...
    function loadDropsIntoMap(target, rawEntries) {
        ...
        for (const rawEntry of rawEntries) {
            ...
            if (itemType.length === 0)
                continue;
            if (!itemDatabase.hasItem(itemType))
                continue;
            const x = Number(rawEntry.x);
            const y = Number(rawEntry.y);
    ...
    function sanitizeDropCreate(data, worldName) {
        ...
        if (!itemDatabase.hasItem(itemType))
            return null;
        if (!itemDatabase.isDropableItem(itemType))
            return null;
        const x = Number(data.x);
        const y = Number(data.y);
        if (!config.isPositionInWorldBounds(x, y))
    ...
    function cleanDropIdList(rawIds, maxIds = config.maxBulkDropPickupIds) {
        const result = [];
        const seen = new Set();
    ...
    function getDropStackGridFromDrop(drop) {
        if (!isRecord(drop))
            return null;
    ...
    function sanitizeBulkDropPickup(data, worldName, player) {
        ...
        if (sameTile) {
            let stackGrid = config.getTransactionGrid(data, "stack_grid_x", "stack_grid_y") ||
